 ...
 */
window.dgFootVersion = 'v591'; console.log('[demigod] foot v591-core loaded');
(function(){
...
function cta(){/*dg-cta-prefetch*/
  ...
  qa('a.premium-btn.is-talent,a.is-talent.premium-btn,a.is-talent').forEach(function(a){
    if(a.closest('#dg-bar,nav,header')) return;
  ...
  qa('a.premium-btn.is-job,a.is-job.premium-btn,a.is-job').forEach(function(a){
    if(a.closest('nav,header')&&!a.closest('.hero-actions,#dg-bar')) return;
    wireCta(a, 'engineer');
  });
  // dual path: hard rebuild to exactly one hire + one looking (Webflow often ships two hire buttons)
  (function ensureHeroDual(){
    ...
    if (host.dataset.dgDual === '1') {
      var hOk = qa('a[data-dg-cta="hire"]', host).length === 1;
      var tOk = qa('a[data-dg-cta="talent"]', host).length === 1;
      var only = qa('a[data-dg-cta]', host).length === 2;
      needRebuild = !(hOk && tOk && only);
    }
    if (needRebuild) {
      host.innerHTML = '';
      var hire = document.createElement('a');
      hire.className = 'premium-btn w-button is-talent';
      hire.setAttribute('data-dg-cta', 'hire');
      var look = document.createElement('a');
      look.className = 'premium-btn w-button is-job';
      look.setAttribute('data-dg-cta', 'talent');
      // build equal cards with hints up-front (ctaHints is then a no-op)
      hire.innerHTML = '<span class="dg-cta-label"></span><span class="dg-cta-hint"></span>';
      look.innerHTML = '<span class="dg-cta-label"></span><span class="dg-cta-hint"></span>';
      var w1 = document.createElement('div'); w1.className = 'dg-cta-wrap dg-in';
      var w2 = document.createElement('div'); w2.className = 'dg-cta-wrap dg-in';
      w1.appendChild(hire); w2.appendChild(look);
      host.appendChild(w1); host.appendChild(w2);
      hire.dataset.dgHinted = '1';
      look.dataset.dgHinted = '1';
      host.dataset.dgDual = '1';
    }
    var hireA = q('.hero-actions a[data-dg-cta="hire"]');
    var lookA = q('.hero-actions a[data-dg-cta="talent"]');
    if (hireA) {
      wireCta(hireA, 'startup');
      var hl = hireA.querySelector('.dg-cta-label');
      var hh = hireA.querySelector('.dg-cta-hint');
      if (hl) hl.textContent = COPY.ctaFounder;
      if (hh) hh.textContent = COPY.ctaHireHint;
    }
    if (lookA) {
      wireCta(lookA, 'engineer');
      var ll = lookA.querySelector('.dg-cta-label');
      var lh = lookA.querySelector('.dg-cta-hint');
      if (ll) ll.textContent = COPY.ctaEngineer;
      if (lh) lh.textContent = COPY.ctaTalentHint;
    }
    [hireA, lookA].forEach(function(a){
      if (!a) return;
      ...
      var wrap = a.closest('.dg-cta-wrap');
      if (wrap) {
        wrap.style.setProperty('display', 'block', 'important');
        wrap.style.setProperty('opacity', '1', 'important');
        wrap.classList.add('dg-in');
      }
    });
  })();
...
var DG_ART={
  bg:'https://litter.catbox.moe/g3atvs.jpg',
  hero:'https://litter.catbox.moe/mallsw.jpg',
  aperture:'https://litter.catbox.moe/60807c.jpg'
};
function injectNightHero(){
  ...
  stage.innerHTML=
    ...
    '<div class="dg-art-panel">'+
      '<img class="dg-art-img" src="'+DG_ART.hero+'" alt="" width="1440" height="1080" decoding="async" fetchpriority="high"/>'+
...
/* v591 Frege-night institutional — phosphor + serif + motion + dual CTA */
":root{--dg-night:#03140d;--dg-deep:#06271a;--dg-field:#075f3a;--dg-green:#08a05d;--dg-signal:#10c674;--dg-phosphor:#a6ffcb;--dg-paper:#f3f0e7;--dg-paper-mute:#bdc9bf;--dg-ink:#071d13;--dg-rule:rgba(166,255,203,.28);--dg-rule-paper:rgba(3,20,13,.2)}"
+"html,body{background:var(--dg-night)!important;color:var(--dg-paper)!important;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace!important}"
+"body::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:9997;opacity:.07;background-image:url(https://litter.catbox.moe/g3atvs.jpg);background-size:cover;background-position:center;mix-blend-mode:screen;animation:dgGrain 12s steps(6) infinite}"
...
+"#dg-night-stage .dg-grain{position:absolute;inset:0;opacity:.35;background:url(https://litter.catbox.moe/g3atvs.jpg) center/cover;mix-blend-mode:soft-light;animation:dgGrain 14s steps(8) infinite}"
...
+"#dg-night-stage .dg-art-caption{position:absolute;left:0;right:0;bottom:0;padding:.45rem .7rem;font-size:.62rem;letter-spacing:.12em;text-transform:uppercase;color:var(--dg-phosphor);background:linear-gradient(transparent,rgba(3,20,13,.92));font-family:ui-monospace,Menlo,Consolas,monospace}"
...
+".dg-eyebrow,#dg-eyebrow{font-family:ui-monospace,Menlo,Consolas,monospace!important;font-size:.72rem!important;letter-spacing:.14em!important;text-transform:uppercase!important;color:var(--dg-signal)!important;margin:0 0 1.1rem!important;opacity:.9}"
+".hero-section h1,.header h1,.hero-title{position:relative;z-index:2;text-shadow:none!important;letter-spacing:-.035em!important;max-width:14ch!important;font-family:Georgia,'Iowan Old Style','Times New Roman',serif!important;font-size:clamp(2.8rem,7.2vw,4.6rem)!important;font-weight:430!important;line-height:.98!important;margin:.2rem 0 1rem!important;color:var(--dg-paper)!important;text-transform:none!important}"
...
+".hero-section p,.header p,.subheading,.hero-description{position:relative;z-index:2;max-width:42ch!important;font-size:1.02rem!important;line-height:1.55!important;color:var(--dg-paper-mute)!important;margin:0 0 1.15rem!important;font-family:ui-monospace,Menlo,Consolas,monospace!important}"
+".hero-badge,.badge-text{display:none!important}"
+"#dg-hero-chips,.dg-trust-line{position:relative;z-index:2;display:block!important;margin:0 0 1.25rem!important;color:rgba(166,255,203,.75)!important;font-size:.82rem!important;letter-spacing:.04em;background:transparent!important;font-family:ui-monospace,Menlo,Consolas,monospace!important}"
+"#dg-path-nudge{display:none!important}"
/* dual path frege-bracket cards */
+".hero-actions,.hero-actions.dg-path-pair{position:relative;z-index:2;display:grid!important;grid-template-columns:1fr 1fr!important;gap:.85rem!important;max-width:34rem!important;width:100%!important;margin:.5rem 0 0!important}"
+".hero-actions .dg-cta-wrap{display:block!important;min-width:0!important}"
+".hero-actions .dg-cta-wrap a,.hero-actions a.premium-btn,.hero-actions a.button,.hero-actions a.w-button,.hero-actions a[data-dg-cta]{box-sizing:border-box!important;display:flex!important;flex-direction:column!important;align-items:flex-start!important;justify-content:center!important;gap:.25rem!important;width:100%!important;min-height:76px!important;padding:1.05rem 1.15rem!important;border-radius:4px!important;font-weight:600!important;font-size:.95rem!important;line-height:1.25!important;text-decoration:none!important;font-family:ui-monospace,Menlo,Consolas,monospace!important;cursor:pointer!important;transition:transform .25s ease,box-shadow .35s ease,border-color .25s ease,background .25s ease!important}"
...
+".hero-actions a[data-dg-cta=talent] .dg-cta-label,.hero-actions a[data-dg-cta=talent] *,.hero-actions a.is-job .dg-cta-label{color:var(--dg-paper)!important}"
...
+"section.trust-section,section:has(.steps-grid),section:has(.pricing-grid),body>section:not(.modal-overlay):not(#startup-modal):not(#jobseeker-modal),.w-section{display:block!important;visibility:visible!important;opacity:1!important;background:var(--dg-night)!important;background-image:none!important;color:var(--dg-paper)!important;padding:3.25rem 1.25rem!important;border-top:1px solid var(--dg-rule)!important}"
+"section:has(.roles-grid),[data-dg-hidden=roles-simplify]{display:none!important}"
+".trust-header h2,.heading_tertiary,section h2,.step-title{color:var(--dg-paper)!important;font-family:Georgia,'Iowan Old Style',serif!important;font-size:clamp(1.6rem,3.5vw,2.35rem)!important;font-weight:430!important;letter-spacing:-.03em!important;margin:.5rem 0 .55rem!important}"
+".trust-header p,.paragraph_large,.step-desc{color:var(--dg-paper-mute)!important;font-size:.95rem!important;max-width:44ch;font-family:ui-monospace,Menlo,Consolas,monospace!important}"
...
+".step-num{color:var(--dg-phosphor)!important;font-size:.72rem!important;font-weight:700!important;letter-spacing:.14em!important;font-family:ui-monospace,Menlo,Consolas,monospace!important}"
...
+".pricing-card h3{color:var(--dg-signal)!important;letter-spacing:.12em!important;font-size:.7rem!important;text-transform:uppercase!important;font-family:ui-monospace,Menlo,Consolas,monospace!important}"
+".pricing-card .pricing-amount,.pricing-card .price,.pricing-card > div > div{color:var(--dg-paper)!important;font-family:Georgia,serif!important}"
+".pricing-card a.premium-btn,.pricing-card a.button{margin-top:1.1rem!important;min-height:48px!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:4px!important;background:rgba(8,160,93,.4)!important;color:var(--dg-phosphor)!important;border:1px solid rgba(166,255,203,.45)!important;padding:.7rem 1.2rem!important;font-family:ui-monospace,Menlo,Consolas,monospace!important}"
...
+"#dg-aperture figcaption{font-size:.65rem;letter-spacing:.12em;padding:.55rem .75rem;color:var(--dg-phosphor);font-family:ui-monospace,Menlo,Consolas,monospace;border-top:1px solid var(--dg-rule);background:rgba(3,20,13,.9)}"
...
+"#dg-copyright,footer [class*=copyright],footer .text-color_secondary{color:rgba(189,201,191,.55)!important;font-size:.78rem!important}"
...
window.__dgFootVer='591';console.log('Demigod v591');
