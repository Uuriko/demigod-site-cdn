/*dg-foot-v833-core*/
/**
 * v833 replaces broken no-JavaScript route/form affordances with native Home and email links.
 * v832 keeps the authored no-JavaScript mobile hero heading in normal document flow.
 * v831 deletes the obsolete IX unhide stack after retiring Webflow's two no-op GSAP interactions.
 * v830 retires the redundant full-DOM contact scrub after the Webflow source fix; raw honesty is release-gated.
 * v829 keeps the saved same-tab wizard step synchronized with the visible step.
 * v828 removes dormant form analytics; Webflow remains the form submission transport.
 * v827 prefers the newest Events API config and probes older mirrors only as fallback.
 * v826 retires the orphan, offline public chatroom and its unused route aliases.
 * v825 makes wizard Close mean close, preserving the existing same-tab draft without a surprise email modal.
 * v824 keeps the H1 height cap without clipping and deletes seven unreachable legacy Events UI helpers.
 * v823 caps the pre-runtime authored H1 to its final box, removing the desktop description shift.
 * v822 removes head-owned hero/nav geometry from the late runtime stylesheet and avoids downloading desktop hero art below 901px.
 * v821 restores the existing homepage process/pricing, links its sample match + SF directory, and removes the hidden blog teaser.
 ...
 * Ship: freeze off → foot-cdn-publish → cm6-paste → truth --require-match
 ...
 */
window.dgFootVersion = 'v833'; console.log('[demigod] foot v833-core loaded');
(function(){
...
function injectNightHero(){
  ...
  stage.innerHTML=
    ...
    '<div class="dg-art-panel">'+
      '<picture class="dg-art-picture"><source media="(min-width:901px)" srcset="'+DG_ART.hero+'"/>'+
...
var DG_PAGES = {
  ...
  status: {
    ...
    html:
      '<ul class="dg-p-list">' +
      '<li><strong>Live:</strong> dual CTAs, one-question WIZ, 90-day outcome, tech matching with humans in the loop, product pages, reviewed event/startup submissions, email follow-up</li>' +
...
window.__dgFootVer='833';console.log('Demigod v833');
