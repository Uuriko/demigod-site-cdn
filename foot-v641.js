 ...
 */
window.dgFootVersion = 'v641'; console.log('[demigod] foot v641-core loaded');
(function(){
...
/* v640: public Events Bot API for live /?p=events (HTTPS tunnel; localhost blocked on prod).
   window.DG_EVENTS_BOT_API = base (.../api/events-bot) or any subpath. localtunnel needs
   Bypass-Tunnel-Reminder for browser UAs or probe gets HTML 511 interstitial. */
function dgEventsBotBases() {
  var raw = '';
  try {
    raw = String((typeof window !== 'undefined' && window.DG_EVENTS_BOT_API) || '').trim();
  } catch (e0) {}
  raw = raw
    .replace(/\/+$/, '')
    .replace(
  ...
  var list = [
    raw,
    /* public tunnel (localtunnel; subdomain can rotate — override with window.DG_EVENTS_BOT_API) */
    'https://serious-badger-25.loca.lt/api/events-bot',
...
function eventsBotCycleMount(root) {
  ...
  async function load() {
    var bases = typeof dgEventsBotBases === 'function' ? dgEventsBotBases() : [];
    for (var i = 0; i < bases.length; i++) {
      var ep = bases[i] + '/lifecycle';
      try {
        var r = await dgEventsBotFetch(ep, { method: 'GET', signal: AbortSignal.timeout(3500) });
        if (!r.ok) continue;
        var j = await r.json();
        if (!j || !j.ok) continue;
...
function eventsBotOffersMount(root) {
  ...
  async function probe() {
    ...
    for (var i = 0; i < bases.length; i++) {
      var base = bases[i];
      try {
        var r = await dgEventsBotFetch(base + '/health', {
          method: 'GET',
        ...
        offerUrl = base + '/offer';
        try {
          var c = await dgEventsBotFetch(base + '/offers', {
            signal: AbortSignal.timeout(3500),
          }).then(function (x) {
            return x.json();
          });
...
function eventsBotExtraMount(root) {
  ...
  async function ensureBase() {
    ...
    for (var i = 0; i < bases.length; i++) {
      try {
        var r = await dgEventsBotFetch(bases[i] + '/health', {
          method: 'GET',
          signal: AbortSignal.timeout(3500),
        });
        if (r.ok) {
...
window.__dgFootVer='641';console.log('Demigod v641');
