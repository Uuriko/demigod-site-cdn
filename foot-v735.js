/*dg-foot-v735-core*/
/**
 * v735 make referral reward timing, individual cash, and hiring-partner credit terms honest.
 ...
 */
window.dgFootVersion = 'v735'; console.log('[demigod] foot v735-core loaded');
(function(){
...
function talentProofPrompt(skills, canUpload){
  if(talentIsTechnical(skills))return {q:'Resume or technical work sample?',h:(canUpload?'Upload PDF/Word or paste':'Paste')+' a GitHub, project, or shareable work link. Shared only after both sides approve.'};
  return {q:'Resume or work sample?',h:(canUpload?'Upload PDF/Word or paste':'Paste')+' a portfolio, writing, campaign, or shareable work link. Shared only after both sides approve.'};
}
...
function dgFormAnalytics(kind, key, event) {
  try {
    ...
    var stepMap = {
      ...
      experience:'work', 'sf-bay':'constraints', resume:'resume',
...
// explicit force unhide for critical keys (90day, review, first fields) to fix vis=0 / hasReview false / has90 false
  ...
  // reopen helper — show() calls this instead of rebuild (v194)
    ...
    ['contact-email','company-name','company-stage','role-title','stack-needs','90day-outcome','salary-range','full-name','seeker-email','linkedin-url','skills-stack','experience','sf-bay'].forEach(function(n){ var el=form.querySelector('[name="'+n+'"],[id="'+n+'"]'); if(el && (cfg.optional||[]).indexOf(n)<0){ el.required=true; if(el.type==='checkbox') el.setAttribute('required','required'); }});
...
if(nativeResume){
  ...
  if(!rw){
    ...
    rw.innerHTML='<label class="w-form-label" for="resume-url">Resume or profile link *</label><input class="w-input" type="url" id="resume-url" name="resume-url" autocomplete="url" maxlength="2048" required placeholder="https://… shareable Drive or Dropbox link"><p class="dg-resume-hint">Paste a shareable Drive or Dropbox link.</p>';
...
var DG_PAGES = {
  ...
  faq: {
    ...
    html:
      ...
      '<details class="dg-p-det"><summary>What do I need as talent?</summary><p>Name, email, core strengths, work highlights, and a resume or profile link. LinkedIn is optional. Free forever — outreach only on real fits.</p></details>' +
...
function eventsBotPublicInviteMount(root) {
  ...
  dgEventsBotPickBase(4000)
    ...
    .then(function (j) {
      ...
      if (bodyEl) {
        bodyEl.innerHTML =
          (ev.outcome ? '<p class="dg-p-note"><strong>Outcome:</strong> ' + String(ev.outcome).replace(/</g, '&lt;') + '</p>' : '') +
          (ev.venue && ev.venue.name
            ? '<p class="dg-p-note"><strong>Venue:</strong> ' + String(ev.venue.name).replace(/</g, '&lt;') + '</p>'
            : '');
      }
      if (form && !ev.rsvpOpen) {
...
window.__dgFootVer='735';console.log('Demigod v735');
