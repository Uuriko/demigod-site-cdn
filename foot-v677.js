 ...
 */
window.dgFootVersion = 'v677'; console.log('[demigod] foot v677-core loaded');
(function(){
...
var WIZ_CFG={
  startup:{
    ...
    welcome:{t:'Demigod',b:'A short brief. One role. One clear 90-day outcome.',btn:'Start the brief'},
...
function paintDualPathH1(el) {
  if (!el) return;
  el.classList.remove('dg-cyber-host', 'dg-hero-hold');
...
function hero(){
  // Brand hold: show cyber "Demigod" longer, then morph to dual-path H1
  var HOLD_MS = Number(window.__dgHeroHoldMs) || 4800;
  qa('.hero-section h1,.hero-title,.header h1').forEach(function(e){
    ...
    if (phase === 'hold') return; // timer already running — leave Demigod up
    var reduce =
      (window.matchMedia && window.matchMedia('(prefers-reduced-motion:reduce)').matches) ||
      document.documentElement.classList.contains('dg-reduce');
    if (reduce) {
    ...
    e.setAttribute('data-dg-hero-phase', 'hold');
    e.classList.add('dg-hero-hold');
    paintCyberWord(e, 'Demigod');
    window.clearTimeout(e._dgHeroHoldTimer);
    e._dgHeroHoldTimer = window.setTimeout(function () {
      try {
        e.classList.add('dg-hero-morph-out');
        window.setTimeout(function () {
          paintDualPathH1(e);
          e.classList.add('dg-reveal', 'dg-in', 'dg-hero-morph-in');
        }, 420);
      } catch (_) {
        paintDualPathH1(e);
      }
...
function mob(){if(q('#dg-bar'))return;var b=document.createElement('nav');b.id='dg-bar';b.setAttribute('aria-label','Mobile actions');b.innerHTML='<a class="dg-h" href="/?wiz=startup" data-demigod-modal="startup" data-dg-cta="hire" aria-label="Demigod — open startup brief">'+COPY.ctaFounder+'</a><a class="dg-j" href="/?wiz=engineer" data-demigod-modal="jobseeker" data-dg-cta="talent" aria-label="I\'m looking — open talent profile">'+COPY.ctaEngineer+'</a>';document.body.appendChild(b)}
...
function cta(){/*dg-cta-prefetch*/
  ...
  function wireCta(a, kind){
    ...
    a.setAttribute('aria-label', hire ? 'Demigod — open startup brief' : "I'm looking — open talent profile");
  ...
  (function ensureHeroDual(){
    ...
    function mk(kind){
      ...
      a.setAttribute('aria-label', kind==='hire' ? 'Demigod — open startup brief' : "I'm looking — open talent profile");
...
/* cyber Demigod brand hold — letter float + soft glitch */
+"@keyframes dgCyberFloat{0%,100%{transform:translateY(0) scale(1)}40%{transform:translateY(-5px) scale(1.04)}70%{transform:translateY(2px) scale(.98)}}"
+"@keyframes dgCyberGlitch{0%,90%,100%{transform:translate(0,0);filter:none;opacity:1}92%{transform:translate(-1px,1px);filter:hue-rotate(40deg);opacity:.85}94%{transform:translate(2px,-1px);opacity:1}96%{transform:translate(-2px,0);filter:hue-rotate(-30deg)}}"
+"@keyframes dgCyberScan{0%{background-position:0 -120%}100%{background-position:0 220%}}"
+"@keyframes dgHeroMorphOut{to{opacity:0;transform:translateY(-12px) scale(.96);filter:blur(2px)}}"
+"@keyframes dgHeroMorphIn{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:none}}"
+".dg-cyber-host,.dg-cyber-word{font-family:var(--dg-cyber)!important;font-weight:700!important;letter-spacing:.14em!important;text-transform:uppercase!important;color:var(--dg-phosphor)!important;text-shadow:0 0 12px rgba(166,255,203,.45),0 0 28px rgba(16,198,116,.28)!important}"
+".hero-section h1.dg-cyber-host,.header h1.dg-cyber-host{font-size:clamp(2.6rem,8vw,4.8rem)!important;line-height:1.05!important;max-width:16ch!important}"
+".dg-cyber-word{display:inline-flex!important;flex-wrap:wrap!important;gap:0.02em!important;position:relative!important}"
+".dg-cyber-ch{display:inline-block!important;animation:dgCyberFloat 1.35s ease-in-out infinite;animation-delay:calc(var(--i,0)*70ms);will-change:transform}"
+".dg-cyber-ch:nth-child(3n){animation-name:dgCyberFloat,dgCyberGlitch;animation-duration:1.35s,2.8s;animation-timing-function:ease-in-out,steps(2,end)}"
+".dg-cta-cyber.dg-cyber-host,.dg-cta-cyber .dg-cyber-word{font-size:1.05em!important;letter-spacing:.16em!important}"
+".dg-cta-cyber .dg-cyber-ch{animation-duration:1.1s}"
+".hero-section h1.dg-hero-morph-out,.header h1.dg-hero-morph-out{animation:dgHeroMorphOut .4s ease forwards!important}"
+".hero-section h1.dg-hero-morph-in,.header h1.dg-hero-morph-in{animation:dgHeroMorphIn .75s cubic-bezier(.16,1,.3,1) both!important}"
+".dg-reduce .dg-cyber-ch,.dg-reduce .dg-cyber-word{animation:none!important;transform:none!important;filter:none!important}"
  ...
  qa('a,button,summary,h1,h2,h3,h4,p,li,span,[role="button"],[role="link"],[role="navigation"],[class*="nav-link"],[class*="step-title"],[class*="process-step"],[class*="pricing"],[class*="guarantee"]').forEach(function(el){
    ...
    if (staleHire.test(t) && !/^Demigod\.?$/i.test(t)) {
      ...
      if (el.matches('a,button')) {
        el.setAttribute('aria-label', 'Demigod — open startup brief');
...
window.__dgFootVer='677';console.log('Demigod v677');
