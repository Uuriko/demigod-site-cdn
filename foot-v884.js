/*dg-foot-v884-core*/
window.dgFootVersion = 'v884'; console.log('[demigod] foot v884-core loaded');
(function(){
...
var MATCH_DISCLOSURE='Software compares outcomes, skills, location, and compensation. A human decides what to propose. Nothing is sent automatically.';
var COPY={
heroSub:'Start with one role, a clear success profile, and real constraints. '+MATCH_DISCLOSURE+' Names move only after both sides say yes.',
badge:'// SF STARTUP TALENT · SOFTWARE + HUMAN REVIEW',
heroTrustLine:'Bay Area · 10% of first-year base salary when a hire starts · Free for talent · Mutual interest only',
...
startupBody:'One role, a clear success profile, and real constraints. '+MATCH_DISCLOSURE,
engineerH2:'Talent profile',
engineerBody:'One private profile. '+MATCH_DISCLOSURE,
...
trustSteps:['You send a brief or profile','Software compares · a human proposes','Both approve → intro'],
...
var WIZ_THANKS={
  startup:{head:'Brief in',lead:'Someone reads every submission. We only propose when the fit is real — both sides still have to say yes.',steps:['We read the role, success profile, and constraints','If fit looks real, we confirm the interview path','Both sides approve before any intro']},
...
var WIZ_CFG={
  startup:{
    steps:[['welcome'],['company-name'],['company-stage'],['role-title'],['90day-outcome'],['stack-needs'],['work-location'],['salary-range'],['contact-email'],['__submit__'],['__thanks__']],
  ...
  engineer:{
    ...
    welcome:{t:'Join the talent network',b:'About 2 minutes · one question at a time. Private profile, free forever. Shared with a startup only after you approve an intro. Press Enter to continue.',btn:'Start my profile'},
    thanks:ENGINEER_OK,
    optional:[]
  }
};
...
var WIZ_Q={
  startup:{
    ...
    '90day-outcome':{q:'By day 90, what will success look like?',h:'One measurable result or milestone — not a task list.'},
  ...
  engineer:{
    ...
    'sf-bay':{q:'Which work setup are you open to?',h:'We compare this with each role’s actual work arrangement.'},
    ...
    '__submit__':{q:'Ready?',h:'Edit anything first. Private until you approve an intro. Free for talent — no blasts.'}
  }
};
...
function startupOutcomePrompt(role){
  ...
  return /\b(engineer(?:ing)?|developer|software|product|designer?|data|ai|ml|security|infrastructure|devops|architect)\b/i.test(role)
    ? {q:'By day 90, what should your '+role+' have shipped or improved?',h:'One concrete release, migration, reliability, adoption, or delivery result.'}
    : {q:'By day 90, what measurable result should your '+role+' have delivered?',h:'One hiring, pipeline, revenue, retention, launch, or operating result.'};
}
...
// explicit force unhide for critical keys (90day, review, first fields) to fix vis=0 / hasReview false / has90 false
    ...
    if (key === 'welcome') {
      ...
      if(kind==='startup'&&key==='role-title'&&answers['company-name']){
        var co=String(answers['company-name']).trim().slice(0,40);
        qd={q:'What role are you hiring for at '+co+'?',h:qd.h};
      }
...
if(!st.querySelector('[name="90day-outcome"]')){var od=document.createElement('div');od.className='dg-field-wrap';od.innerHTML='<label class="w-form-label" for="90day-outcome">By day 90, what will success look like? *</label><textarea class="w-input" id="90day-outcome" name="90day-outcome" rows="2" required placeholder="One measurable result or milestone"></textarea>';var skg2=sk&&(sk.closest('.form-field-group')||sk.parentElement);if(skg2&&skg2.parentElement&&skg2!==st){skg2.insertAdjacentElement('afterend',od);}else{(sk&&sk.parentElement||st).appendChild(od);}}
...
function hero(){
  ...
  qa('.step-card').forEach(function(card,i){
    ...
    var steps=[
      ['Define success','Hiring teams share one measurable first-90-day result, then the must-haves and constraints.'],
      ['Compare, then review','Software compares outcomes and constraints. A human confirms the interview path and decides what to propose.'],
  ...
  qa('.pricing-card').forEach(function(card){
    qa('div,li,p,span',card).forEach(function(el){
      ...
      if(/90-day replacement/i.test(tx)) el.textContent='Success by day 90';
...
function scrubTimeClaims(){
  ...
  qa('*').forEach(function(el){
    ...
    if (bareGuarantee.test(txt) && txt.length < 200) {
      el.textContent = '10% of first-year base salary when a hire starts · success by day 90';
      return;
    }
    if (/pre-vetted|Dedicated talent partner|90-?\s*day replacement/i.test(txt) && txt.length < 160 && !el.closest('#startup-modal,#jobseeker-modal')) {
      el.textContent = txt
        ...
        .replace(/90-?\s*day replacement guarantee/ig,'success by day 90');
      return;
    }
    if (bad.test(txt)) {
...
function scrubBadStaticClaims(){
  try{
    ...
    nodes.forEach(function(tn){
      var v=tn.nodeValue||'';
      var nv=v
        .replace(/90-?\s*day\s+replacement\s+guarantee/ig,'success by day 90')
    ...
    qa('div,li,p,span').forEach(function(el){
      if(el.children&&el.children.length)return;
      var tx=(el.textContent||'').trim();
      if(/^success by day 90/i.test(tx)||/^outcome-focused matching$/i.test(tx))return;
      if(/replacement guarantee/i.test(tx)&&tx.length<80){
        el.textContent='success by day 90';
      }
    });
  }catch(e){}
}
...
function scrubStaticLabels(){
  ...
  qa('.modal-intro,#startup-modal .modal-intro,#jobseeker-modal .modal-intro').forEach(function(el){
    ...
    if(/curated SF candidates|match you to SF startups/i.test(t)){
      el.textContent=/startup|role|comp/i.test(t)
        ? 'Share the role, a clear success profile, and real constraints. Software compares the evidence.'
  ...
  qa('.pricing-card div,.pricing-card li,.pricing-grid div,.pricing-grid li').forEach(function(el){
    ...
    else if(/90-?\s*day replacement guarantee/i.test(t)) el.textContent='success by day 90';
  ...
  function honestStaticCopy(t){
    ...
    if(staleCandidateGuarantee.test(out)){staleCandidateGuarantee.lastIndex=0;out=out.replace(staleCandidateGuarantee,'success by day 90');}
    staleGuaranteedPeriod.lastIndex=0;
    if(staleGuaranteedPeriod.test(out)){staleGuaranteedPeriod.lastIndex=0;out=out.replace(staleGuaranteedPeriod,'success by day 90');}
    ...
    if(staleGuarantee.test(out)){staleGuarantee.lastIndex=0;out=out.replace(staleGuarantee,'success by day 90');}
    staleGuaranteePeriod.lastIndex=0;
    if(staleGuaranteePeriod.test(out)){staleGuaranteePeriod.lastIndex=0;out=out.replace(staleGuaranteePeriod,'success by day 90');}
    staleComplimentaryReplacement.lastIndex=0;
    if(staleComplimentaryReplacement.test(out)){staleComplimentaryReplacement.lastIndex=0;out=out.replace(staleComplimentaryReplacement,'success by day 90');}
    staleConditionalReplacement.lastIndex=0;
    if(staleConditionalReplacement.test(out)){staleConditionalReplacement.lastIndex=0;out=out.replace(staleConditionalReplacement,'success by day 90');}
    staleFitReplacement.lastIndex=0;
    if(staleFitReplacement.test(out)){staleFitReplacement.lastIndex=0;out=out.replace(staleFitReplacement,'success by day 90');}
    staleShouldReplacement.lastIndex=0;
    if(staleShouldReplacement.test(out)){staleShouldReplacement.lastIndex=0;out=out.replace(staleShouldReplacement,'success by day 90');}
    staleOfferedReplacement.lastIndex=0;
    if(staleOfferedReplacement.test(out)){staleOfferedReplacement.lastIndex=0;out=out.replace(staleOfferedReplacement,'success by day 90');}
    staleSearchRestart.lastIndex=0;
    if(staleSearchRestart.test(out)){staleSearchRestart.lastIndex=0;out=out.replace(staleSearchRestart,'success by day 90');}
    staleNextSearchFree.lastIndex=0;
    if(staleNextSearchFree.test(out)){staleNextSearchFree.lastIndex=0;out=out.replace(staleNextSearchFree,'success by day 90');}
    staleReplacementCredit.lastIndex=0;
    if(staleReplacementCredit.test(out)){staleReplacementCredit.lastIndex=0;out=out.replace(staleReplacementCredit,'success by day 90');}
    staleFeeWaiver.lastIndex=0;
    if(staleFeeWaiver.test(out)){staleFeeWaiver.lastIndex=0;out=out.replace(staleFeeWaiver,'success by day 90');}
    staleFeeCarryover.lastIndex=0;
    if(staleFeeCarryover.test(out)){staleFeeCarryover.lastIndex=0;out=out.replace(staleFeeCarryover,'success by day 90');}
    staleFeeBackGuarantee.lastIndex=0;
    if(staleFeeBackGuarantee.test(out)){staleFeeBackGuarantee.lastIndex=0;out=out.replace(staleFeeBackGuarantee,'success by day 90');}
    staleMoneyBackGuarantee.lastIndex=0;
    if(staleMoneyBackGuarantee.test(out)){staleMoneyBackGuarantee.lastIndex=0;out=out.replace(staleMoneyBackGuarantee,'success by day 90');}
    staleReplacementBenefit.lastIndex=0;
    if(staleReplacementBenefit.test(out)){staleReplacementBenefit.lastIndex=0;out=out.replace(staleReplacementBenefit,'success by day 90');}
    staleReplacementSafetyNet.lastIndex=0;
    if(staleReplacementSafetyNet.test(out)){staleReplacementSafetyNet.lastIndex=0;out=out.replace(staleReplacementSafetyNet,'success by day 90');}
    staleReplacementInsurance.lastIndex=0;
    if(staleReplacementInsurance.test(out)){staleReplacementInsurance.lastIndex=0;out=out.replace(staleReplacementInsurance,'success by day 90');}
    staleReplacementReassurance.lastIndex=0;
    if(staleReplacementReassurance.test(out)){staleReplacementReassurance.lastIndex=0;out=out.replace(staleReplacementReassurance,'success by day 90');}
    staleReplacementService.lastIndex=0;
    if(staleReplacementService.test(out)){staleReplacementService.lastIndex=0;out=out.replace(staleReplacementService,'success by day 90');}
    staleTalentGuarantee.lastIndex=0;
    if(staleTalentGuarantee.test(out)){staleTalentGuarantee.lastIndex=0;out=out.replace(staleTalentGuarantee,'success by day 90');}
    staleOutcomeGuarantee.lastIndex=0;
    if(staleOutcomeGuarantee.test(out)){staleOutcomeGuarantee.lastIndex=0;out=out.replace(staleOutcomeGuarantee,'success by day 90');}
    staleRiskFreeHiring.lastIndex=0;
    if(staleRiskFreeHiring.test(out)){staleRiskFreeHiring.lastIndex=0;out=out.replace(staleRiskFreeHiring,'success by day 90');}
    staleReversedGuarantee.lastIndex=0;
    if(staleReversedGuarantee.test(out)){staleReversedGuarantee.lastIndex=0;out=out.replace(staleReversedGuarantee,'success by day 90');}
    staleBackedReplacement.lastIndex=0;
    if(staleBackedReplacement.test(out)){staleBackedReplacement.lastIndex=0;out=out.replace(staleBackedReplacement,'success by day 90');}
    staleExpenseReplacement.lastIndex=0;
    if(staleExpenseReplacement.test(out)){staleExpenseReplacement.lastIndex=0;out=out.replace(staleExpenseReplacement,'success by day 90');}
    return out;
  }
  ...
  qa('a,button,summary,h1,h2,h3,h4,p,li,span,[role="button"],[role="link"],[role="navigation"],[class*="nav-link"],[class*="step-title"],[class*="process-step"],[class*="pricing"],[class*="guarantee"]').forEach(function(el){
    ...
    if (staleCandidateGuarantee.test(t)) {
      staleCandidateGuarantee.lastIndex = 0;
      el.textContent = t.replace(staleCandidateGuarantee, 'success by day 90');
      return;
    }
    staleGuaranteedPeriod.lastIndex = 0;
    if (staleGuaranteedPeriod.test(t)) {
      staleGuaranteedPeriod.lastIndex = 0;
      el.textContent = t.replace(staleGuaranteedPeriod, 'success by day 90');
      return;
    }
    staleIncludedReplacement.lastIndex = 0;
    ...
    if (staleGuarantee.test(t)) {
      staleGuarantee.lastIndex = 0;
      el.textContent = t.replace(staleGuarantee, 'success by day 90');
      return;
    }
    staleGuaranteePeriod.lastIndex = 0;
    if (staleGuaranteePeriod.test(t)) {
      staleGuaranteePeriod.lastIndex = 0;
      el.textContent = t.replace(staleGuaranteePeriod, 'success by day 90');
      return;
    }
    staleComplimentaryReplacement.lastIndex = 0;
    if (staleComplimentaryReplacement.test(t)) {
      staleComplimentaryReplacement.lastIndex = 0;
      el.textContent = t.replace(staleComplimentaryReplacement, 'success by day 90');
      return;
    }
    staleConditionalReplacement.lastIndex = 0;
    if (staleConditionalReplacement.test(t)) {
      staleConditionalReplacement.lastIndex = 0;
      el.textContent = t.replace(staleConditionalReplacement, 'success by day 90');
      return;
    }
    staleFitReplacement.lastIndex = 0;
    if (staleFitReplacement.test(t)) {
      staleFitReplacement.lastIndex = 0;
      el.textContent = t.replace(staleFitReplacement, 'success by day 90');
      return;
    }
    staleShouldReplacement.lastIndex = 0;
    if (staleShouldReplacement.test(t)) {
      staleShouldReplacement.lastIndex = 0;
      el.textContent = t.replace(staleShouldReplacement, 'success by day 90');
      return;
    }
    staleOfferedReplacement.lastIndex = 0;
    if (staleOfferedReplacement.test(t)) {
      staleOfferedReplacement.lastIndex = 0;
      el.textContent = t.replace(staleOfferedReplacement, 'success by day 90');
      return;
    }
    staleSearchRestart.lastIndex = 0;
    if (staleSearchRestart.test(t)) {
      staleSearchRestart.lastIndex = 0;
      el.textContent = t.replace(staleSearchRestart, 'success by day 90');
      return;
    }
    staleNextSearchFree.lastIndex = 0;
    if (staleNextSearchFree.test(t)) {
      staleNextSearchFree.lastIndex = 0;
      el.textContent = t.replace(staleNextSearchFree, 'success by day 90');
      return;
    }
    staleReplacementCredit.lastIndex = 0;
    if (staleReplacementCredit.test(t)) {
      staleReplacementCredit.lastIndex = 0;
      el.textContent = t.replace(staleReplacementCredit, 'success by day 90');
      return;
    }
    staleFeeWaiver.lastIndex = 0;
    if (staleFeeWaiver.test(t)) {
      staleFeeWaiver.lastIndex = 0;
      el.textContent = t.replace(staleFeeWaiver, 'success by day 90');
      return;
    }
    staleFeeCarryover.lastIndex = 0;
    if (staleFeeCarryover.test(t)) {
      staleFeeCarryover.lastIndex = 0;
      el.textContent = t.replace(staleFeeCarryover, 'success by day 90');
      return;
    }
    staleFeeBackGuarantee.lastIndex = 0;
    if (staleFeeBackGuarantee.test(t)) {
      staleFeeBackGuarantee.lastIndex = 0;
      el.textContent = t.replace(staleFeeBackGuarantee, 'success by day 90');
      return;
    }
    staleMoneyBackGuarantee.lastIndex = 0;
    if (staleMoneyBackGuarantee.test(t)) {
      staleMoneyBackGuarantee.lastIndex = 0;
      el.textContent = t.replace(staleMoneyBackGuarantee, 'success by day 90');
      return;
    }
    staleReplacementBenefit.lastIndex = 0;
    if (staleReplacementBenefit.test(t)) {
      staleReplacementBenefit.lastIndex = 0;
      el.textContent = t.replace(staleReplacementBenefit, 'success by day 90');
      return;
    }
    staleReplacementSafetyNet.lastIndex = 0;
    if (staleReplacementSafetyNet.test(t)) {
      staleReplacementSafetyNet.lastIndex = 0;
      el.textContent = t.replace(staleReplacementSafetyNet, 'success by day 90');
      return;
    }
    staleReplacementInsurance.lastIndex = 0;
    if (staleReplacementInsurance.test(t)) {
      staleReplacementInsurance.lastIndex = 0;
      el.textContent = t.replace(staleReplacementInsurance, 'success by day 90');
      return;
    }
    staleReplacementReassurance.lastIndex = 0;
    if (staleReplacementReassurance.test(t)) {
      staleReplacementReassurance.lastIndex = 0;
      el.textContent = t.replace(staleReplacementReassurance, 'success by day 90');
      return;
    }
    staleReplacementService.lastIndex = 0;
    if (staleReplacementService.test(t)) {
      staleReplacementService.lastIndex = 0;
      el.textContent = t.replace(staleReplacementService, 'success by day 90');
      return;
    }
    staleBackedReplacement.lastIndex = 0;
    if (staleBackedReplacement.test(t)) {
      staleBackedReplacement.lastIndex = 0;
      el.textContent = t.replace(staleBackedReplacement, 'success by day 90');
      return;
    }
    staleExpenseReplacement.lastIndex = 0;
    if (staleExpenseReplacement.test(t)) {
      staleExpenseReplacement.lastIndex = 0;
      el.textContent = t.replace(staleExpenseReplacement, 'success by day 90');
      return;
    }
    staleTalentGuarantee.lastIndex = 0;
    if (staleTalentGuarantee.test(t)) {
      staleTalentGuarantee.lastIndex = 0;
      el.textContent = t.replace(staleTalentGuarantee, 'success by day 90');
      return;
    }
    staleOutcomeGuarantee.lastIndex = 0;
    if (staleOutcomeGuarantee.test(t)) {
      staleOutcomeGuarantee.lastIndex = 0;
      el.textContent = t.replace(staleOutcomeGuarantee, 'success by day 90');
      return;
    }
    staleRiskFreeHiring.lastIndex = 0;
    if (staleRiskFreeHiring.test(t)) {
      staleRiskFreeHiring.lastIndex = 0;
      el.textContent = t.replace(staleRiskFreeHiring, 'success by day 90');
      return;
    }
    staleReversedGuarantee.lastIndex = 0;
    if (staleReversedGuarantee.test(t)) {
      staleReversedGuarantee.lastIndex = 0;
      el.textContent = t.replace(staleReversedGuarantee, 'success by day 90');
    }
  });
...
var DG_PAGES = {
  how: {
    ...
    desc: 'Share a brief or profile. Software compares outcomes and constraints; a human decides what to propose. Both sides approve before an intro.',
    html:
      '<p class="dg-p-lead">A match has three gates. Names and contact details move only after all three pass.</p>' +
      '<ol class="dg-p-list">' +
      '<li><strong>Brief or profile</strong> — startups define success first, then the must-haves and constraints; talent sends one profile. Real inputs, not keywords.</li>' +
      '<li><strong>Software compares; a human decides</strong> — outcomes, skills, location, and compensation are compared. Before proposing a match, a person confirms the interview path and target timing. Nothing is sent automatically.</li>' +
  ...
  faq: {
    title: 'FAQ',
    doc: 'FAQ · Demigod',
    desc: 'Answers about Demigod’s SF matching, submissions, 10% success fee, free talent profiles, privacy, success by day 90, and mutual intros.',
    html:
      ...
      '<details class="dg-p-det"><summary>What does success by day 90 mean?</summary><p>One measurable result the hire should deliver in their first 90 days. We match against that result — not a keyword soup or generic JD.</p></details>' +
  ...
  hire: {
    title: 'Hire talent',
    doc: 'Hire · Demigod',
    desc: 'Submit a brief. Software compares outcomes and constraints; a human decides what to propose. 10% of first-year base salary only when a hire starts.',
    html:
      '<p class="dg-p-lead">Tell us the role, what success looks like by day 90, and the constraints that matter. A person reads every brief — we only propose when the fit is real, and both sides still have to say yes.</p>' +
      '<ul class="dg-p-list">' +
      '<li><strong>Send a ~2-min brief</strong> — role, one measurable success milestone, must-haves, work mode, and cash band.</li>' +
      '<li><strong>Software compares, a human reviews</strong> — against the outcome and real constraints. If fit looks real, we confirm the interview path before talent sees it.</li>' +
  ...
  talent: {
    ...
    html:
      ...
      '</ul>' +
      '<p class="dg-p-note">No board spam or cold-LinkedIn blasts. What we ask: the work you want next, evidence from work you have done, availability, work setup, and a cash band — enough to match against success by day 90, not keywords. <a href="/how" data-dg-page="how">How it works →</a> · <a href="/faq" data-dg-page="faq">FAQ →</a></p>',
  },
  contact: {
    title: 'Contact',
    doc: 'Contact · Demigod',
...
window.__dgFootVer='884';console.log('Demigod v884');
