 ...
 */
window.dgFootVersion = 'v611'; console.log('[demigod] foot v611-core loaded');
(function(){
...
var DG_PAGES = {
  ...
  mud: {
    ...
    desc: 'Lightweight text MUD set in San Francisco — SoMa, Market, Embarcadero. Create a character, walk, talk. Classic verbs.',
    html:
      '<p class="dg-p-lead">A <strong>full-page</strong> text MUD set in <strong>San Francisco</strong> (SoMa / Market Street / Embarcadero). Walk rooms that name-check the Bay — Stripe energy, Ritual coffee, South Park, Ferry Building — as <em>flavor only</em>, not partnerships.</p>' +
      '<ul class="dg-p-list">' +
      ...
      '<p class="dg-p-note">Solo offline in-browser. Multiplayer: <code>node demigod-mud-app.mjs</code> on :3461. SF startup names are setting color — not Demigod clients or endorsements.</p>',
  },
...
(function (root, factory) {
  ...
  /* —— World: SF Night District (SoMa / Market / Embarcadero frege-night) —— */
  var ROOMS = {
    plaza: {
      name: 'Market Street Crossing',
      desc: 'Fog on Market. F-line clang in the distance. A phosphor plaque reads DECISION > VOLUME — next to a peeling sticker for an old Twitter bird and a fresh Stripe atlas pin.',
      exits: { north: 'lobby', east: 'alley', west: 'cafe', south: 'dock' },
      items: ['leaflet'],
    },
    lobby: {
      name: 'Demigod Matching Lobby (SoMa)',
      desc: 'Second-floor loft south of Market. Sealed envelopes only — no open job board. A wall map of the Bay has pins on SoMa, Mission, Dogpatch, and a lonely one in Oakland.',
      exits: { south: 'plaza', east: 'review', up: 'roof' },
      items: ['notebook'],
    },
    review: {
      name: 'Mutual-Approve Booths',
      desc: 'Six quiet booths. Evidence packets only. Someone left a Notion printout titled "90-day outcome — no keyword soup."',
      exits: { west: 'lobby', north: 'archive' },
      items: ['stamp'],
    },
    archive: {
      name: 'Outcome Archive',
      desc: 'Shelves of SF first-quarter ships: activation curves, infra cost downs, GTM loops. A spine reads "Airbnb host trust · 2012 lessons" — history, not a partnership claim.',
      exits: { south: 'review' },
      items: ['tome'],
    },
    cafe: {
      name: 'Ritual Coffee Nook',
      desc: 'Mission-adjacent energy: cold brew, laptop stickers for Linear, Figma, and a battered YC backpack. Chalkboard: open tables — humans only, no auto-DM.',
      exits: { east: 'plaza', north: 'garden' },
      items: ['mug'],
    },
    garden: {
      name: 'South Park Pocket',
      desc: 'Tiny green oval off 2nd & Bryant. Startups once orbiting here still haunt the benches. Fog slides in from the Embarcadero.',
      exits: { south: 'cafe', east: 'lobby' },
      items: ['keycard'],
    },
    alley: {
      name: 'Folsom Alley',
      desc: 'Brick, loading docks, server fans. Graffiti tags read SHIP IT and a faded Uber Eats bike stencil. A side door hums like a Twilio edge box.',
      exits: { west: 'plaza', north: 'lab', south: 'market' },
      items: ['coin'],
    },
    lab: {
      name: 'Stack Lab (Dogpatch edge)',
      desc: 'Whiteboards: latency budgets, Postgres plans, "what would Stripe do at p99?" A rack blinks green. Soldering iron cold next to a Dropbox sticker sheet.',
      exits: { south: 'alley' },
      items: ['cable'],
    },
    market: {
      name: 'Ferry Building Night Market',
      desc: 'Pop-up stalls under string lights — SAMPLE roles only, labeled honest. Someone argues about seed vs Series A comps near the oyster cart.',
      exits: { north: 'alley', west: 'dock' },
      items: ['badge'],
    },
    dock: {
      name: 'Embarcadero Pier',
      desc: 'Bay water, salt air, Bay Bridge lights. A ferry horn answers the night. You can almost see Salesforce Park glowing inland.',
      exits: { north: 'plaza', east: 'market' },
      items: ['shell'],
    },
    roof: {
      name: 'SoMa Roof Deck',
      desc: 'Wind off the bay. Neon and cranes. Sutro Tower is a faint trident. Quiet enough to think about one hire who can ship.',
      exits: { down: 'lobby' },
      items: ['lantern'],
    },
  };
  ...
  var ITEMS = {
    leaflet: { name: 'a phosphor SF leaflet', look: 'Front: create, walk, talk — type HELP. Back: "Set in San Francisco. SF startups named as flavor only — not partners."' },
    notebook: { name: 'a Field Notes pad', look: 'Bought on Valencia. Blank pages for score and 90-day outcomes.' },
    stamp: { name: 'a mutual-approve stamp', look: 'Ink says BOTH SIDES. Standard for intros that are not cold LinkedIn spam.' },
    tome: { name: 'a Bay Area outcome tome', look: 'Case studies from SF: activation, infra, GTM. Metrics only — no name-dropping hires as Demigod wins.' },
    mug: { name: 'a Ritual coffee mug', look: 'Still warm. Lipstick of a founder who said "we need a Stripe-quality PM."' },
    keycard: { name: 'a South Park fob', look: 'Decorative access to quieter rooms. Engraved 2ND & BRYANT.' },
    coin: { name: 'a sample Muni token', look: 'Stamped SAMPLE. Not Clipper. Not a real placement fee.' },
    cable: { name: 'a Cat6 run', look: 'Labeled "for the lab rack — not for your WeWork HDMI crisis."' },
    badge: { name: 'a pilot badge', look: 'White-glove SF pilot. Commercial path still email: potter@trydemigod.com.' },
    shell: { name: 'an Embarcadero shell', look: 'Cold and smooth. Bay Bridge light catches the ridge.' },
    lantern: { name: 'a roof lantern', look: 'Soft green light over SoMa — signal without shouting.' },
  };

  var NPCS = [
    { id: 'hermes', name: 'Hermes', room: 'lobby', look: 'A quiet SoMa coordinator with a phosphor pin and a BART card tucked in their notebook.', lines: [
      'Evidence first. Names later — this is still San Francisco.',
      'Type HELP if you are new to the district. Market Street is south of here.',
      'Both sides approve. No open talent blast like the old boards.',
    ]},
    { id: 'ada', name: 'Ada', room: 'lab', look: 'An engineer tracing p99 latency. Stickers: Cloudflare, GitHub, a tiny Postgres elephant.', lines: [
      'Ship something measurable in 90 days — the SF way when the hype dies down.',
      'I used to watch Stripe-quality systems posts at 1am. Outcome still beats stack fashion.',
      'Dogpatch to SoMa on the T is fine. Just bring the metric.',
    ]},
    { id: 'rio', name: 'Rio', room: 'cafe', look: 'A founder nursing Ritual cold brew. YC backpack half-zipped under the chair.', lines: [
      'I need one hire who owns the outcome — not a ten-name shortlist from a board.',
      'We are not Airbnb-scale. We need someone who has shipped 0→1 in the Bay.',
      'Say HI if you want a warm intro back to Market Street.',
    ]},
    { id: 'kai', name: 'Kai', room: 'dock', look: 'A PM watching ferry lights. Salesforce tower is a glass shard inland.', lines: [
      'Comps conversations here get real fast — SF is expensive and founders know it.',
      'I want a builder who has seen a Series A kitchen and did not flinch.',
    ]},
  ];
  ...
  Engine.prototype.boot = function () {
    ...
    this.print('A lightweight text MUD set in SF (SoMa / Market / Embarcadero). Create a character, walk, talk. Type HELP anytime.', 'sys');
    if (ch) {
      this.char = ch;
      this.mode = 'play';
  ...
  Engine.prototype.handleCreate = function (line) {
    ...
    saveChar(this.char);
    this.mode = 'play';
    this.print('Created ' + this.char.name + ' the ' + c.title + '. You materialize on Market Street Crossing, San Francisco.', 'sys');
    this.cmd('look');
    this.tryJoinMulti();
  };
  ...
  Engine.prototype.cmd = function (raw) {
    ...
    switch (verb) {
      ...
      case 'players':
        ...
        this.print('NPCs still walk SoMa and the Embarcadero. Multiplayer who-list needs the MUD API (:3461).', 'sys');
        return;
      case 'score':
      case 'stats':
      case 'status':
      ...
      case 'exit':
      case 'logout':
        this.print('You fade into the SF fog. Character kept for next visit. Type anything to look again.', 'sys');
        saveChar(ch);
        return;
      case 'restart':
      case 'new':
  ...
  function mount(root, opts) {
    ...
    box.innerHTML =
      ...
      '</form>' +
      '<p class="dg-mud-hint">Set in San Francisco · <code>/mud</code> · classic verbs · saves in this browser</p>' +
      '</div>';
...
window.__dgFootVer='611';console.log('Demigod v611');
