/*dg-foot-v1099-core*/
window.dgFootVersion = 'v1099'; console.log('[demigod] foot v1099-core loaded');
(function(){
...
var DG_PAGES = {
  ...
  bounties: {
    ...
    desc: 'Anyone can list a GitHub repo with a declared bounty. Demigod does not hold funds. Completing a bounty is not a hire or an intro.',
    html:
      '<p class="dg-p-lead"><strong>Anyone can post a repo and a bounty.</strong> Founder or not. You set the amount and the rules. We list it. We do not hold or send the money. Hunt the GitHub item. A listing is not a job post, a public profile, or an intro.</p>' +
      '<p class="dg-p-actions"><a class="hire" href="#dg-bounty-list">List a bounty →</a><a class="talent dg-p-actions-sec" href="#dg-bounty-open">Open bounties →</a></p>' +
      '<ul class="dg-p-list">' +
      '<li><strong>Who:</strong> anyone with a repo and a bounty they will actually pay.</li>' +
      '<li><strong>Declared, not escrow.</strong> Payout is between you and the hunter, however you announced.</li>' +
      '<li><strong>Not matching.</strong> If a bounty later turns into a Demigod hire, the usual 10% of first-year base still applies. This board does not replace that.</li>' +
      '</ul>' +
      '<h2 class="dg-p-h3" id="dg-bounty-open">Open bounties</h2>' +
      '<div id="dg-bounty-live"><p class="dg-p-note">Loading listings…</p></div>' +
      '<h2 class="dg-p-h3" id="dg-bounty-list">List a bounty</h2>' +
      '<form id="dg-bounty-form" class="dg-ev-form">' +
      '<label class="dg-ev-lab" for="dg-bounty-repo">GitHub repo or issue URL</label>' +
      '<input class="dg-ev-in" id="dg-bounty-repo" name="repo" type="url" required placeholder="https://github.com/owner/repo" autocomplete="off" />' +
      '<label class="dg-ev-lab" for="dg-bounty-title">Title</label>' +
      '<input class="dg-ev-in" id="dg-bounty-title" name="title" maxlength="120" placeholder="What should someone ship" />' +
      '<label class="dg-ev-lab" for="dg-bounty-amount">Amount</label>' +
      '<input class="dg-ev-in" id="dg-bounty-amount" name="amount" inputmode="decimal" placeholder="e.g. 500" />' +
      '<label class="dg-ev-lab" for="dg-bounty-currency">Currency</label>' +
      '<input class="dg-ev-in" id="dg-bounty-currency" name="currency" value="USD" maxlength="8" />' +
      '<label class="dg-ev-lab" for="dg-bounty-pays">What pays</label>' +
      '<textarea class="dg-ev-in" id="dg-bounty-pays" name="pays" rows="3" maxlength="600" placeholder="Merged PR, accepted patch, written spec…"></textarea>' +
      '<label class="dg-ev-lab" for="dg-bounty-email">Your email</label>' +
      '<input class="dg-ev-in" id="dg-bounty-email" name="email" type="email" required autocomplete="email" />' +
      '<button type="submit" class="dg-ev-submit">Email this listing</button>' +
      '<p class="dg-ev-msg" id="dg-bounty-msg" role="status" aria-live="polite">Opens mail to potter@trydemigod.com with your listing. No account. We do not take custody of the bounty.</p>' +
      '</form>' +
      '<p class="dg-p-note"><a href="/how" data-dg-page="how">How matching works →</a> · <a href="/pricing" data-dg-page="pricing">Hire fee →</a> · <a href="/legal" data-dg-page="legal">Privacy →</a></p>',
  },
  map: {
...
var DG_BOUNTY_SEED = [
  {kind:'item',name:'docs: add CONTRIBUTING screenshot of GitHub web edit flow',repo:'Uuriko/dasha-desk',itemUrl:'https://github.com/Uuriko/dasha-desk/issues/8',amount:25,currency:'USD',pays:'A merged PR that adds the screenshot the issue asks for.',blurb:'Also listed on getdasha.com/bounties.'},
  {kind:'project',name:'dasha desk',repo:'Uuriko/dasha-desk',itemUrl:null,amount:50,currency:'USD',pays:'Merged pull requests and useful issues on the desk or Studio.',blurb:'Also listed on getdasha.com/bounties.'}
];
function bountyRender(root, listings) {
  var host = root && root.querySelector('#dg-bounty-live');
  if (!host) return;
  if (!listings.length) {
    host.innerHTML = '<p class="dg-p-note">None live yet. List one below. A human adds it here so this page does not become an unreviewed dump.</p>';
    return;
  }
  host.innerHTML = '<ul class="dg-p-list">' + listings.map(function (it) {
    var href = it.itemUrl || (it.repo ? ('https://github.com/' + it.repo) : '');
    var pay = (it.amount != null && it.amount !== '') ? (esc(String(it.amount)) + ' ' + esc(it.currency || 'USD')) : 'undeclared';
    var title = href ? ('<a href="' + esc(href) + '" rel="noopener" target="_blank">' + esc(it.name) + '</a>') : esc(it.name);
    var extra = it.source === 'dasha' ? ' Also on getdasha.com/bounties.' : '';
    return '<li><strong>' + title + '</strong> · ' + pay + '. ' + esc(it.pays || it.blurb || '') + extra + '</li>';
  }).join('') + '</ul>';
}
function bountyLoadFeeds(root) {
  bountyRender(root, DG_BOUNTY_SEED.map(function (it) { return bountyNormalize(it, 'dasha'); }).filter(Boolean));
  ...
  Promise.all(urls.map(function (u) {
    ...
    DG_BOUNTY_SEED.forEach(function (it) { add(it, 'dasha'); });
...
function bountyFormMount(root) {
  ...
  form.addEventListener('submit', function (ev) {
    ...
    var title = String((root.querySelector('#dg-bounty-title') || {}).value || '').trim();
    var amount = String((root.querySelector('#dg-bounty-amount') || {}).value || '').trim();
    var currency = String((root.querySelector('#dg-bounty-currency') || {}).value || 'USD').trim() || 'USD';
    var pays = String((root.querySelector('#dg-bounty-pays') || {}).value || '').trim();
    var email = String((root.querySelector('#dg-bounty-email') || {}).value || '').trim();
    var msg = root.querySelector('#dg-bounty-msg');
    if (!/^https:\/\/github\.com\/[^/]+\/[^/]+/i.test(repo)) {
      if (msg) msg.textContent = 'Use a full https://github.com/owner/repo URL.';
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      if (msg) msg.textContent = 'Add a real email so we can list you.';
      return;
    }
    var body = [
      'Repo: ' + repo,
      'Title: ' + (title || '(none)'),
      'Amount: ' + (amount ? amount + ' ' + currency : 'undeclared'),
      'What pays: ' + (pays || '(none)'),
      'From: ' + email,
      '',
      'Declared bounty, not escrow. Poster pays the hunter directly.'
    ].join('\n');
    location.href = 'mailto:potter@trydemigod.com?subject=' + encodeURIComponent('Demigod bounty: ' + (title || repo)) + '&body=' + encodeURIComponent(body);
    if (msg) msg.textContent = 'Mail opened. We add it to the open list after a human reads it.';
  });
}
...
window.__dgFootVer='1099';console.log('Demigod v1099');
