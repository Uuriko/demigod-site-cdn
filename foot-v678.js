 ...
 */
window.dgFootVersion = 'v678'; console.log('[demigod] foot v678-core loaded');
(function(){
...
function scheduleHeroBrandHold(el) {
  if (!el) return;
  var HOLD_MS = Number(window.__dgHeroHoldMs) || 9000; // Demigod stays up ~9s
  var FADE_MS = Number(window.__dgHeroFadeMs) || 1100; // slow crossfade
  ...
  el.style.transition = 'opacity ' + FADE_MS + 'ms cubic-bezier(.4,0,.2,1), transform ' + FADE_MS + 'ms cubic-bezier(.4,0,.2,1), filter ' + FADE_MS + 'ms ease';
...
/* cyber Demigod — soft entrance then gentle breathe (no harsh glitch/jump) */
+"@keyframes dgCyberIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}"
+"@keyframes dgCyberBreathe{0%,100%{transform:translateY(0);text-shadow:0 0 8px rgba(166,255,203,.35),0 0 18px rgba(16,198,116,.18)}50%{transform:translateY(-2px);text-shadow:0 0 14px rgba(166,255,203,.55),0 0 28px rgba(16,198,116,.28)}}"
+"@keyframes dgCyberPulseSoft{0%,100%{opacity:1}50%{opacity:.88}}"
+".dg-cyber-host,.dg-cyber-word{font-family:var(--dg-cyber)!important;font-weight:700!important;letter-spacing:.16em!important;text-transform:uppercase!important;color:var(--dg-phosphor)!important;-webkit-text-fill-color:var(--dg-phosphor)!important;text-shadow:0 0 12px rgba(166,255,203,.4),0 0 26px rgba(16,198,116,.22)!important}"
/* beat serif + paper-span rules that caused color/size flash */
+".hero-section h1.dg-hero-hold,.header h1.dg-hero-hold,.hero-section h1.dg-cyber-host,.header h1.dg-cyber-host{font-family:var(--dg-cyber)!important;font-size:clamp(2.6rem,8vw,4.8rem)!important;font-weight:700!important;line-height:1.12!important;letter-spacing:.12em!important;max-width:18ch!important;min-height:2.55em!important;color:var(--dg-phosphor)!important;text-transform:uppercase!important}"
...
+".dg-cyber-word{display:inline-flex!important;flex-wrap:nowrap!important;gap:0.03em!important;position:relative!important;align-items:baseline!important}"
+".dg-cyber-ch{display:inline-block!important;opacity:0;animation:dgCyberIn .95s cubic-bezier(.22,1,.36,1) calc(var(--i,0)*95ms) both,dgCyberBreathe 3.2s ease-in-out calc(0.95s + var(--i,0)*95ms) infinite;will-change:transform,opacity}"
/* one letter soft pulse only — calmer than full glitch */
+".dg-cyber-ch:nth-child(4){animation:dgCyberIn .95s cubic-bezier(.22,1,.36,1) calc(var(--i,0)*95ms) both,dgCyberBreathe 3.2s ease-in-out calc(0.95s + var(--i,0)*95ms) infinite,dgCyberPulseSoft 4.5s ease-in-out 2s infinite}"
+".dg-cta-cyber.dg-cyber-host,.dg-cta-cyber .dg-cyber-word{font-size:1.02em!important;letter-spacing:.14em!important}"
+".dg-cta-cyber .dg-cyber-ch{animation-duration:.85s,2.8s}"
...
window.__dgFootVer='678';console.log('Demigod v678');
