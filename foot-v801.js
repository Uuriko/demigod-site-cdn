 ...
 */
window.dgFootVersion = 'v801'; console.log('[demigod] foot v801-core loaded');
(function(){
...
var WIZ_CFG={
  ...
  engineer:{
    steps:[['welcome'],['full-name'],['seeker-email'],['skills-stack'],['experience'],['sf-bay'],['availability'],['salary-expectation'],['resume'],['__submit__'],['__thanks__']],
...
if(!en.querySelector('[name=salary-expectation]')){var se=document.createElement('div');se.className='dg-field-wrap';se.innerHTML='<label class="w-form-label" for="salary-expectation">Cash compensation range *</label><input class="w-input" type="text" id="salary-expectation" name="salary-expectation" required placeholder="Target base range and equity context">';en.appendChild(se);}
var seIn=en.querySelector('[name=salary-expectation]');if(seIn){seIn.required=true;seIn.placeholder='Target base range and equity context';var seLab=(seIn.closest('.form-field-group,.dg-field-wrap')||en).querySelector('label');if(seLab)seLab.textContent='Cash compensation range *';}
...
'<details><summary>Add an unlisted startup</summary><form id="dg-startup-submit" class="dg-community-form"><p class="dg-p-note">* Required. Submit factual public information; a human reviews it before listing.</p><label>Startup name *<input class="dg-ev-in" name="name" required maxlength="160"></label><label>Website *<input class="dg-ev-in" name="website" type="url" required placeholder="https://"></label><label>SF neighborhood *<input class="dg-ev-in" name="neighborhood" required maxlength="120"></label><label>What does it do? *<textarea class="dg-ev-in" name="description" rows="3" required maxlength="1200"></textarea></label><label>Hiring?<select class="dg-ev-in" name="hiring"><option value="unknown">Not sure</option><option value="yes">Yes</option><option value="no">No</option></select></label><label>Your name *<input class="dg-ev-in" name="submitterName" required maxlength="120" autocomplete="name"></label><label>Your email *<input class="dg-ev-in" name="submitterEmail" type="email" required maxlength="160" autocomplete="email"></label><button class="dg-ev-submit" type="submit">Submit startup for review</button><p class="dg-submit-msg" role="status" aria-live="polite"></p></form></details>'+
...
window.__dgFootVer='801';console.log('Demigod v801');
