 ...
 */
window.dgFootVersion = 'v684'; console.log('[demigod] foot v684-core loaded');
(function(){
...
var WIZ_CFG={
  ...
  engineer:{
    ...
    optional:['phone','links','salary-expectation','why-startups','availability']
  }
};
...
var WIZ_Q={
  ...
  engineer:{
    ...
    'linkedin-url':{q:'LinkedIn URL?',h:'We use this to verify context. Your shipped work still matters most.'},
...
function wizBuild(form, kind) {
  ...
  function showStep(idx) {
    ...
    var fileFallback = fileStep && form.querySelector('[name="' + key + '-url"]');
    if (fileFallback) toShow.push(fileFallback);
    ...
    if (key && key !== 'welcome' && key !== '__submit__' && key !== '__thanks__') {
      ...
      if (el && !isOpt) {
        ...
        if (el.type === 'checkbox' || el.type === 'radio') empty = !el.checked;
          ...
          var fileReady = !!(el.files && el.files.length && String(el.getAttribute('data-value') || '').trim());
        ...
        errEl.textContent = el.type === 'file' ? 'Paste a shareable resume link, or wait for the file upload to finish.' : 'Please answer this one — then continue.';
        describedAdd(el, errEl.id || (errEl.id = 'dg-wiz-req-err-' + key));
        if (fileUrl) describedAdd(fileUrl, errEl.id);
        var clearRequired = function(){
          el.style.borderColor = ''; el.removeAttribute('aria-invalid'); describedRemove(el, errEl.id);
          if (fileUrl) describedRemove(fileUrl, errEl.id);
        ...
        if (el.type === 'file') {
          if (el.files && el.files.length && !el.checkValidity()) { el.reportValidity(); el.focus(); return; }
...
+"}"
+"#startup-modal input[type=file]::file-selector-button,#jobseeker-modal input[type=file]::file-selector-button{"
...
window.__dgFootVer='684';console.log('Demigod v684');
