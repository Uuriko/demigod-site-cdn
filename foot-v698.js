 ...
 */
window.dgFootVersion = 'v698'; console.log('[demigod] foot v698-core loaded');
(function(){
...
function talentExperiencePrompt(skills){
  return /\b(engineer(?:ing)?|software|developer|frontend|backend|fullstack|mobile|data|machine learning|ml|ai|devops|infrastructure|security)\b/i.test(String(skills||''))
...
function dgFormAnalytics(kind, key, event) {
  try {
    ...
    var stepMap = {
      ...
      'stack-needs':'skills', '90day-outcome':'outcome', 'contact-email':'contact',
      'full-name':'profile', 'seeker-email':'contact', 'linkedin-url':'profile',
      experience:'work', 'sf-bay':'constraints', resume:'profile',
...
// explicit force unhide for critical keys (90day, review, first fields) to fix vis=0 / hasReview false / has90 false
    ...
    if (key === 'welcome') {
      ...
      qa('.dg-wiz-edit', rev).forEach(function(btn){
        btn.onclick = function(){
          ...
          if (idx >= 0 && idx < steps.length) { reviewReturn = current; showStep(idx); }
        };
      });
  ...
  backBtn.onclick = function(ev){ ev&&ev.preventDefault(); reviewReturn = -1; if (current > 0) showStep(current - 1); };
...
var DG_PAGES = {
  ...
  legal: {
    ...
    html:
      ...
      '<p>A temporary same-tab draft is kept in your browser session and cleared after confirmed submission. Aggregate form analytics retain only form, step, event, broad device type, hour, and count for up to 30 days; the payload excludes answers and contact details.</p>' +
...
window.__dgFootVer='698';console.log('Demigod v698');
