 ...
 */
window.dgFootVersion = 'v799'; console.log('[demigod] foot v799-core loaded');
(function(){
...
var WIZ_CFG={
  startup:{
    steps:[['welcome'],['company-name'],['company-stage'],['role-title'],['stack-needs'],['90day-outcome'],['work-location'],['salary-range'],['contact-email'],['__submit__'],['__thanks__']],
    welcome:{t:'Hiring brief',b:'A short brief. One role. One clear 90-day outcome. Answers stay in this tab so you can resume; they clear after submission.',btn:'Start the brief'},
    thanks:STARTUP_OK,
    optional:[]
  },
  engineer:{
    steps:[['welcome'],['full-name'],['seeker-email'],['skills-stack'],['experience'],['links'],['sf-bay'],['availability'],['salary-expectation'],['resume'],['__submit__'],['__thanks__']],
    welcome:{t:'Talent profile',b:'One private profile. Free. Shared only when you approve. Answers stay in this tab so you can resume; they clear after submission.',btn:'Start my profile'},
    thanks:ENGINEER_OK,
    optional:['links']
  }
};
...
var WIZ_Q={
  startup:{
    ...
    'work-location':{q:'Where and how can this person work?',h:'Choose the arrangement the role can actually support.'},
    'salary-range':{q:'Target cash compensation range?',h:'A rough base range is enough; add equity context if useful.'},
    ...
    '__submit__':{q:'Look good?',h:'Demigod and its form/email providers process these answers for matching. Identifying details move only after both sides approve.'}
  },
  engineer:{
    ...
    'skills-stack':{q:'What do you do best?',h:'Strengths, craft, domain — product, design, GTM, operations, recruiting, engineering, or something else.'},
    'experience':{q:"What work are you proud of?",h:'Two or three concrete wins beat a long bio.'},
    'sf-bay':{q:'Open to SF Bay startups?',h:'Our focus. Remote is fine when the company is Bay-based.'},
    'availability':{q:'When could you start?',h:'Now, in 2–4 weeks, in 1–3 months, or passively open.'},
    'salary-expectation':{q:'What cash compensation range are you targeting?',h:'Your target, never salary history. A rough base range is enough; add equity context if useful.'},
    'why-startups':{q:'Why startups? (optional)',h:'One line if you want.'},
    'links':{q:'Work link or profile? (optional)',h:'LinkedIn, portfolio, writing, campaign, GitHub, or anything that shows the work.'},
    'phone':{q:'Phone? (optional)',h:'SMS is pending. Email is the main channel.'},
    'resume':{q:'Resume or work link?',h:'Upload a PDF/Word resume or paste a shareable HTTPS resume, portfolio, or work link. Shared only after both sides approve.'},
    '__submit__':{q:'Ready?',h:'Demigod and its form/email providers process these answers for matching. Identifying details move only after both sides approve.'}
  }
};
...
function dgFormAnalytics(kind, key, event) {
  try {
    ...
    var stepMap = {
      ...
      'stack-needs':'skills', '90day-outcome':'outcome', 'work-location':'constraints', 'salary-range':'constraints', 'contact-email':'contact',
      'full-name':'name', 'seeker-email':'contact', 'linkedin-url':'linkedin',
      experience:'work', 'sf-bay':'constraints', availability:'constraints', 'salary-expectation':'constraints', resume:'resume',
    ...
    dgEventsBotPickBase(1500).then(function(picked){
      if (!picked || !picked.base) return;
      return dgEventsBotFetch(picked.base + '/analytics/forms', { method:'POST', headers:{'Content-Type':'application/json'}, body:body, keepalive:true });
    }).catch(function(){});
  } catch (e) {}
}
...
function formAttribution(form){
  ...
  ['utm_source','utm_medium','utm_campaign','utm_content','utm_term','referral','role_id','event_id'].forEach(function(name){
    ...
    if(!value||value.length>120||!/^[A-Za-z0-9][A-Za-z0-9._ -]*$/.test(value))return;
...
if(!st.querySelector('[name=salary-range]')){var w=document.createElement('div');w.id='dg-salary-wrap';w.className='dg-field-wrap';w.innerHTML='<label class="w-form-label" for="salary-range">Target cash compensation range? *</label><input class="w-input" type="text" id="salary-range" name="salary-range" required placeholder="e.g. $180-220k + equity">';if(sa&&sa.parentElement)sa.parentElement.insertBefore(w,sa.nextSibling);else st.querySelector('[type=submit],.w-button')?.parentElement?.insertBefore(w,st.querySelector('[type=submit],.w-button'))}else{var salary=st.querySelector('[name=salary-range]');salary.required=true;ph(salary,'e.g. $180-220k + equity');qa('label',st).forEach(function(l){if(/salary|comp range/i.test(l.textContent||''))l.textContent='Target cash compensation range? *'})}
...
if(nativeResume){
  ...
  if(!rw){
    ...
    rw.innerHTML='<label class="w-form-label" for="resume-url">Resume or work link *</label><input class="w-input" type="url" id="resume-url" name="resume-url" autocomplete="url" maxlength="2048" required placeholder="https://… resume, portfolio, or work sample"><p class="dg-resume-hint">Paste one shareable HTTPS link.</p>';
...
ph(en.querySelector('[name=skills-stack]'),'e.g. Product strategy, Figma, growth marketing');var skIn=en.querySelector('[name=skills-stack]');if(skIn)skIn.required=true;charCount(en.querySelector('[name=skills-stack]'),400);var ex=en.querySelector('[name=experience]');if(ex&&ex.tagName==='SELECT'){var ta=document.createElement('textarea');ta.className='w-input';ta.name='experience';ta.id='experience';ta.rows=3;ta.placeholder='e.g. Shipped v1 at seed startup; led growth from 0→$1M ARR';ta.required=true;(ex.closest('.w-select')||ex).replaceWith(ta)}else if(ex){ex.required=true;ph(ex,'e.g. Shipped v1 at seed startup; led growth from 0→$1M ARR')}charCount(en.querySelector('[name=experience]'),600);if(!en.querySelector('[name=links]')){var experienceWrap=ex&&(ex.closest('.form-field-group,.dg-field-wrap,.w-file-upload')||ex.parentElement);var lw=document.createElement('div');lw.id='dg-links-wrap';lw.className='dg-field-wrap';lw.innerHTML='<label class="w-form-label" for="links">Work link or profile (optional)</label><input class="w-input" type="url" id="links" name="links" maxlength="2048" placeholder="https://… LinkedIn, GitHub, portfolio, or work sample">';if(experienceWrap&&experienceWrap.parentElement)experienceWrap.parentElement.insertBefore(lw,experienceWrap.nextSibling);else{var sub=en.querySelector('[type=submit],.w-button');sub?.parentElement?.insertBefore(lw,sub)}}var sf=en.querySelector('[name=sf-bay]');
...
if(!en.querySelector('[name=salary-expectation]')){var se=document.createElement('div');se.className='dg-field-wrap';se.innerHTML='<label class="w-form-label" for="salary-expectation">Cash compensation range *</label><input class="w-input" type="text" id="salary-expectation" name="salary-expectation" required placeholder="$180–220k base + equity">';en.appendChild(se);}
var seIn=en.querySelector('[name=salary-expectation]');if(seIn){seIn.required=true;seIn.placeholder='$180–220k base + equity';var seLab=(seIn.closest('.form-field-group,.dg-field-wrap')||en).querySelector('label');if(seLab)seLab.textContent='Cash compensation range *';}
    ...
    panel.innerHTML=
      ...
      '<nav id="dg-legal-links" aria-label="Footer navigation">'+
        ...
        '<div class="dg-footer-group" role="group" aria-labelledby="dg-footer-network">'+
          ...
          '<a href="/?p=partners" data-dg-page="partners">Partners</a>'+
        '</div>'+
        '<div class="dg-footer-group" role="group" aria-labelledby="dg-footer-explore">'+
          ...
          '<a href="/?p=map" data-dg-page="map">SF startup map</a>'+
...
function dgMapEventsHtml(){return '<p class="dg-p-lead"><strong>Browse SF startup activity and event venue leads in one place.</strong> Toggle either layer, move or resize the search circle, then switch to the list for what falls inside it.</p>'+
'<p class="dg-ev-pill" role="note">San Francisco · open data · venue availability not verified</p>'+
'<div id="dg-startup-map"><p class="dg-p-lead">Loading the SF startup and venue map…</p></div>'+
'<section id="dg-community-listings" class="dg-community-submit" aria-labelledby="dg-community-listings-title"><h2 id="dg-community-listings-title" class="dg-p-h3">Community listings</h2><p class="dg-p-note">Loading reviewed events and startups…</p></section>'+
'<section class="dg-community-submit" aria-labelledby="dg-community-title"><h2 id="dg-community-title" class="dg-p-h3">Submit and manage</h2><p class="dg-p-lead">Submit an SF event or add an unlisted startup. Event destinations can be Demigod, Luma, Partiful, or Demigod plus one external platform. Every submission is reviewed before publishing.</p>'+
'<div class="dg-submit-grid"><details open><summary>Submit an event</summary><form id="dg-event-submit" class="dg-community-form"><label>Event title *<input class="dg-ev-in" name="title" required maxlength="120"></label><label>Organizer name *<input class="dg-ev-in" name="organizerName" required maxlength="120" autocomplete="name"></label><label>Organizer email *<input class="dg-ev-in" name="organizerEmail" type="email" required maxlength="160" autocomplete="email"></label><label>Date and time (your local timezone) *<input class="dg-ev-in" name="startsAt" type="datetime-local" required></label><label>Format *<select class="dg-ev-in" name="format" required><option value="">Select</option><option value="in-person">In person</option><option value="online">Online</option><option value="hybrid">Hybrid</option></select></label><label>Venue or neighborhood<input class="dg-ev-in" name="venue" maxlength="180"></label><label>Who is it for?<input class="dg-ev-in" name="audience" maxlength="240"></label><label>Capacity<input class="dg-ev-in" name="seats" type="number" min="1" step="1"></label><label>Publish destination *<select class="dg-ev-in" name="destination" required><option value="demigod">trydemigod.com</option><option value="luma">Luma only</option><option value="partiful">Partiful only</option><option value="demigod+luma">trydemigod.com + Luma</option><option value="demigod+partiful">trydemigod.com + Partiful</option></select></label><label>Existing Luma or Partiful link<input class="dg-ev-in" name="externalUrl" type="url" inputmode="url" maxlength="500" placeholder="https://lu.ma/… or https://partiful.com/…" aria-describedby="dg-event-url-help"></label><p id="dg-event-url-help" class="dg-p-note">Required for any Luma or Partiful destination. The link must match the selected platform.</p><label>Event details<textarea class="dg-ev-in" name="details" rows="4" maxlength="2000"></textarea></label><button class="dg-ev-submit" type="submit">Submit event</button><p class="dg-submit-msg" role="status" aria-live="polite"></p></form></details>'+
'<details><summary>Add an unlisted startup</summary><form id="dg-startup-submit" class="dg-community-form"><label>Startup name *<input class="dg-ev-in" name="name" required maxlength="160"></label><label>Website<input class="dg-ev-in" name="website" type="url" placeholder="https://"></label><label>SF neighborhood<input class="dg-ev-in" name="neighborhood" maxlength="120"></label><label>What does it do?<textarea class="dg-ev-in" name="description" rows="3" maxlength="1200"></textarea></label><label>Hiring?<select class="dg-ev-in" name="hiring"><option value="unknown">Not sure</option><option value="yes">Yes</option><option value="no">No</option></select></label><label>Your name *<input class="dg-ev-in" name="submitterName" required maxlength="120" autocomplete="name"></label><label>Your email *<input class="dg-ev-in" name="submitterEmail" type="email" required maxlength="160" autocomplete="email"></label><button class="dg-ev-submit" type="submit">Submit startup</button><p class="dg-submit-msg" role="status" aria-live="polite"></p></form></details>'+
'<details><summary>Manage my submissions</summary><p class="dg-p-note">This browser keeps private management keys for event and startup submissions made here.</p><div id="dg-event-manage" aria-live="polite"><p class="dg-p-note">No submissions saved in this browser yet.</p></div></details></div><p class="dg-p-note">Submitting does not publish, book a venue, contact guests, or create an external account. Existing Luma or Partiful events can be linked during submission or later management.</p></section>'}
var DG_PAGES = {
  how: {
    title: 'How it works',
  ...
  faq: {
    ...
    html:
      ...
      '<details class="dg-p-det"><summary>What do I need as talent?</summary><p>Name, email, core strengths, work highlights, availability, cash compensation range, and one resume or work link. Free forever — outreach only on real fits.</p></details>' +
  ...
  partners: {
    ...
    desc: 'Approved referral rewards accrue after 90 days and the related client fee is paid and retained. Written terms apply.',
    html:
  ...
  map: {
    title: 'SF startup & events map',
    doc: 'SF startup & events map · Demigod',
    desc: 'Browse privacy-safe SF startup activity and public venue leads; submit events, add unlisted startups, and manage event details.',
    html: dgMapEventsHtml(),
  },
  method: {
    title: 'Method',
    doc: 'Method · Demigod',
...
function pageCss() {
  ...
  if (existing && existing.getAttribute('data-v') === '753') return;
  if (existing) existing.remove();
  var s = document.createElement('style');
  s.id = 'dg-page-css';
  s.setAttribute('data-v', '753');
  s.textContent =
...
function communitySubmissionsMount(root) {
  ...
  async function renderListings() {
    ...
    if (!box) return;
    try {
      var settled = await Promise.allSettled([get('/community-events'), get('/community-startups')]);
      var eventsOk = settled[0].status === 'fulfilled', startupsOk = settled[1].status === 'fulfilled';
  ...
  async function renderManage() {
    ...
    if (!loaded.length) { manage.innerHTML = '<p class="dg-p-note">No reachable submissions are saved in this browser.</p>'; return; }
    manage.innerHTML = loaded.map(function (item) {
      if (item.startup) {
        ...
        return '<form class="dg-manage-card" data-id="' + esc(company.id) + '"><h3>' + esc(company.name) + '</h3><p class="dg-p-note">Status: ' + esc(company.status) + '</p>' +
          '<label>Startup name<input class="dg-ev-in" name="name" value="' + esc(company.name) + '" maxlength="160" required></label><label>Website<input class="dg-ev-in" name="website" type="url" value="' + esc(company.website || '') + '" placeholder="https://"></label><label>SF neighborhood<input class="dg-ev-in" name="neighborhood" value="' + esc(company.neighborhood || '') + '" maxlength="120"></label>' +
          '<label>Hiring?<select class="dg-ev-in" name="hiring">' + [['yes','Yes'],['no','No'],['unknown','Not sure']].map(function (option) { return '<option value="' + option[0] + '"' + (company.hiring === option[0] ? ' selected' : '') + '>' + option[1] + '</option>'; }).join('') + '</select></label><label>What should people know?<textarea class="dg-ev-in" name="description" rows="3" maxlength="1200">' + esc(company.description || '') + '</textarea></label><button class="dg-ev-submit" type="submit">Save changes</button> ' +
      ...
      return '<form class="dg-manage-card" data-id="' + esc(ev.id) + '"><h3>' + esc(ev.title) + '</h3><p class="dg-p-note">Status: ' + esc(ev.status) + ' · destination: ' + esc(ev.destination) + '</p>' +
        ...
        '<label>Existing Luma or Partiful link<input class="dg-ev-in" name="externalUrl" type="url" value="' + esc(ev.externalUrl || '') + '" placeholder="https://"></label><label>Details<textarea class="dg-ev-in" name="details" rows="3" maxlength="2000">' + esc(ev.details || '') + '</textarea></label><button class="dg-ev-submit" type="submit">Save changes</button> ' +
  ...
  renderManage().catch(function () { manage.innerHTML = '<p class="dg-p-note">Saved submissions could not load right now.</p>'; });
  renderListings();
}
...
function pageCtas(id) {
  ...
  if (id === 'events') return back;
  if (id === 'mud' || id === 'vesper') return back;
  if (id === 'contact' || id === 'legal' || id === 'partners') return hire + talent + back;
  return hire + talent + back;
}
var DG_STARTUP_MAP_ASSET='https://cdn.jsdelivr.net/gh/Uuriko/demigod-site-cdn@5d8467d214363a36d169f5f47883aad3812a5116/startup-map-latest.js';
...
function startupMapMount(root) {
  ...
  if(!dgStartupMapLoading){
    dgStartupMapLoading=new Promise(function(resolve,reject){
      ...
      script.async=true;
      script.onload=function(){window.DemigodStartupMap?resolve():reject(new Error('Startup map API missing'));};
      script.onerror=function(){reject(new Error('Startup map script failed'));};
  ...
  dgStartupMapLoading.then(function(){if(host.isConnected)window.DemigodStartupMap.mount(host);}).catch(function(){
    ...
    host.innerHTML='<p class="dg-p-lead" role="alert">The map could not load.</p><button type="button" class="dg-page-x" id="dg-startup-map-retry">Retry</button>';
...
function openPage(id, push) {
  ...
  if (id === 'events' || id === 'map') root.classList.add('dg-page-events', 'dg-page-map');
...
window.__dgFootVer='799';console.log('Demigod v799');
