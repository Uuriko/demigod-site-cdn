 ...
 */
window.dgFootVersion = 'v564'; console.log('[demigod] foot v564-core loaded');
(function(){
...
var COPY={
heroSub:'Send a brief or your profile. A human introduces you only when both sides want it.',
badge:'SF Bay Area',
heroTrustLine:'Private · Both approve · 10% on hire',
ctaFounder:"I'm hiring",
ctaEngineer:'Looking for work',
navCta:"I'm hiring",
navCtaTalent:'Looking for work',
startupH2:"I'm hiring",
startupBody:'Tell us the role and what success looks like in 90 days. A human reads every brief.',
engineerH2:'Looking for work',
engineerBody:'One private profile. Shared only when both of you say yes.',
...
footerTag:'Human match for SF startups · both approve · 10% on hire',
trustKicker:'How it works',
trustSteps:['You send a brief or profile','A human finds a fit','Both approve → intro'],
ledgerKicker:'',
ledgerTitle:'',
ledgerRows:[],
...
var WIZ_CFG={startup:{steps:[['welcome'],['contact-email'],['company-name'],['company-stage'],['role-title'],['stack-needs'],['90day-outcome'],['__submit__'],['__thanks__']],welcome:{t:"I'm hiring",b:'One role. What must they ship in 90 days? A human reads every brief.',btn:'Start →'},thanks:STARTUP_OK,total:7,optional:['phone','salary-range','why-this-role','role-jd','timeline','team-size']},engineer:{steps:[['welcome'],['full-name'],['seeker-email'],['linkedin-url'],['skills-stack'],['experience'],['sf-bay'],['__submit__'],['__thanks__']],welcome:{t:'Looking for work',b:'One private profile. Free. Shared only after you both approve.',btn:'Start →'},thanks:ENGINEER_OK,total:7,optional:['phone','links','resume','salary-expectation','why-startups','availability']},partner:{steps:[['welcome'],['partner-type'],['partner-name'],['partner-email'],['partner-phone'],['partner-org'],['referral-plan'],['partner-linkedin'],['partner-notes'],['__submit__'],['__thanks__']],welcome:{t:'Partner',b:'Refer outstanding people. Earn 20% of fee on hire.',btn:'Apply →'},thanks:PARTNER_OK,total:9,optional:['partner-phone','partner-linkedin','partner-notes']}};
...
function hero(){
  qa('.hero-section h1,.hero-title,.header h1').forEach(function(e){
    e.innerHTML='<span class="title-accent-gold">Hire SF talent</span><span class="title-accent-cream"> through a human.</span>';
  });
  qa('.hero-section p,.hero-description,.subheading,.header p').forEach(function(e){
    if(e.closest('form,.w-form')||e.id==='dg-cand-kicker'||e.closest('.dg-candidates,#startup-modal,#jobseeker-modal,#dg-path-pills,#dg-hero-chips,#dg-simple-process'))return;
    var t=e.textContent||'';
  ...
  // Hide noisy sample-role wall only; keep process + pricing (rewritten below)
  ...
  qa('.step-card').forEach(function(card,i){
    ...
    var steps=[
      ['Send a brief or profile','Startups: the role + 90-day outcome. Talent: one private profile.'],
      ['A human finds a fit','No blast lists. We only propose when it looks real.'],
      ['Both approve → intro','You each say yes. Then we connect you.']
    ];
  ...
  qa('.trust-header h2,.trust-section h2,section h2').forEach(function(h){
    ...
    if(/HUMAN-MATCHED|THE PROCESS|How it works/i.test(x) && h.closest('.trust-section,section')) h.textContent='How it works';
  ...
  qa('.trust-header p,.trust-section .paragraph_large').forEach(function(p){
    ...
    if(/spam|profile|Bay Area/i.test(p.textContent||'')) p.textContent='Three steps. No agency theater.';
  });
  qa('.badge-text').forEach(function(b){
    var x=(b.textContent||'').trim();
    if(/^THE PROCESS$/i.test(x)) b.textContent='SIMPLE';
    if(/^PRICING$/i.test(x)) b.textContent='FEE';
    if(/^LIVE ROLES$/i.test(x)) b.textContent='SAMPLES';
  });
  ...
  qa('.pricing-card').forEach(function(card){
    qa('div,li,p,span',card).forEach(function(el){
      ...
      if(/Dedicated talent partner/i.test(tx)) el.textContent='A human reads every brief';
...
/* v564 calm simplify — more space, less chrome, fewer gold ornaments */
"html,body{background:#0B0C0F!important;color:#E8E4DC!important}"
+".nav_container,.nav_left,.nav_right{background:transparent!important}"
+".nav_container{position:sticky!important;top:0!important;z-index:50!important;display:flex!important;align-items:center!important;justify-content:space-between!important;padding:.85rem 1.25rem!important;backdrop-filter:blur(10px);background:rgba(11,12,15,.82)!important;border-bottom:1px solid rgba(255,255,255,.06)!important}"
+".nav_logo .paragraph_large,[data-brand-name]{color:#F4F0E8!important;font-weight:700!important;letter-spacing:.02em}"
+".nav_logo-icon{color:#D4B56A!important}"
+".hero-section,.header{background:radial-gradient(120% 90% at 10% 0%,rgba(212,181,106,.12),transparent 55%),linear-gradient(180deg,#12141A 0%,#0B0C0F 70%)!important;background-image:none!important;min-height:auto!important;display:flex!important;align-items:flex-start!important;padding:clamp(2.5rem,8vh,4.5rem) 1.25rem 2.75rem!important;border-bottom:1px solid rgba(255,255,255,.05)!important}"
+".hero-grid-background,.grid-col-line,.hero-content-right,.statue-frame,.statue-svg,.statue-wrapper{display:none!important}"
+".hero-container,.hero-content-left{max-width:40rem!important;margin:0!important;width:100%!important}"
+".hero-section h1,.header h1,.hero-title{text-shadow:none!important;letter-spacing:-.03em;max-width:14ch!important;font-size:clamp(2.4rem,7vw,3.9rem)!important;font-weight:700!important;line-height:1.02!important;margin:.4rem 0 .9rem!important;color:#F7F3EB!important;animation:none!important}"
+".title-accent-gold{color:#E8D9A8!important}.title-accent-cream,.title-accent-red,.title-accent-blue{color:#F7F3EB!important;display:inline}"
+".hero-section p,.header p,.subheading,.hero-description{max-width:34ch!important;font-size:1.1rem!important;line-height:1.55!important;color:#A8A39A!important;margin:0 0 1.1rem!important;animation:none!important}"
+".hero-badge,.badge-text{display:inline-flex!important;align-items:center!important;gap:.4rem!important;color:#B8B0A4!important;font-size:.7rem!important;font-weight:600!important;letter-spacing:.14em!important;text-transform:uppercase!important;border:1px solid rgba(255,255,255,.1)!important;border-radius:999px!important;padding:.28rem .7rem!important;background:rgba(255,255,255,.03)!important;min-height:auto!important}"
+".badge-dot{width:6px!important;height:6px!important;border-radius:50%!important;background:#D4B56A!important;box-shadow:none!important}"
+"#dg-hero-chips,.dg-trust-line{display:block!important;margin:0 0 1.25rem!important;color:#8E887F!important;font-size:.92rem!important;letter-spacing:.01em;animation:none!important}"
+"#dg-hero-chips .dg-chip{display:none!important}"
+".hero-actions{display:flex!important;flex-wrap:wrap!important;gap:.75rem!important;align-items:stretch!important;margin-top:.15rem;animation:none!important}"
+".hero-actions a.premium-btn,.hero-actions a.button,.hero-actions a.w-button,#dg-bar a.dg-h,#dg-bar a.dg-j,#dg-nav-hire,#dg-nav-talent,a.premium-btn[data-dg-cta],a.button[data-dg-cta]{box-sizing:border-box!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;min-height:52px!important;padding:.85rem 1.35rem!important;border-radius:999px!important;font-weight:650!important;font-size:.95rem!important;line-height:1.2!important;letter-spacing:.01em;text-decoration:none!important;white-space:nowrap!important;cursor:pointer!important;transition:background .15s ease,border-color .15s ease,color .15s ease;box-shadow:none!important;transform:none!important;filter:none!important}"
+".hero-actions a[data-dg-cta=hire],.hero-actions a.is-talent,#dg-bar a.dg-h,#dg-nav-hire,a[data-dg-cta=hire],.button.on-inverse{background:#E8D9A8!important;color:#12141A!important;border:1px solid #E8D9A8!important}"
+".hero-actions a[data-dg-cta=hire]:hover,#dg-nav-hire:hover,a[data-dg-cta=hire]:hover{background:#F3E7C0!important}"
+".hero-actions a[data-dg-cta=talent],.hero-actions a.is-job,#dg-bar a.dg-j,#dg-nav-talent,a[data-dg-cta=talent]{background:transparent!important;color:#E8E4DC!important;border:1px solid rgba(255,255,255,.22)!important}"
+".hero-actions a[data-dg-cta=talent]:hover,a[data-dg-cta=talent]:hover{border-color:rgba(232,217,168,.7)!important;color:#F7F3EB!important}"
+".hero-actions a .btn-label,.hero-actions a .button_label,.hero-actions a .btn-arrow,.hero-actions a svg{display:inline!important;margin:0!important;padding:0!important;background:transparent!important;border:none!important;color:inherit!important;font:inherit!important;opacity:1!important}"
+".hero-actions a .btn-arrow,.hero-actions a svg{display:none!important}"
+"#dg-path-pills{display:flex!important;flex-wrap:wrap!important;gap:.35rem 1.15rem!important;margin:1.35rem 0 0!important;padding-top:1.1rem!important;border-top:1px solid rgba(255,255,255,.08)!important;animation:none!important}"
+"#dg-path-pills a{color:#8E887F!important;font-size:.9rem!important;font-weight:500!important;text-decoration:none!important;border-bottom:none!important;min-height:44px!important;display:inline-flex!important;align-items:center!important}"
+"#dg-path-pills a:hover,#dg-path-pills a:focus-visible{color:#E8D9A8!important;outline:none}"
+"#dg-path-pills a:focus-visible{box-shadow:0 0 0 2px rgba(232,217,168,.45);border-radius:4px}"
...
+"section.trust-section,body>section:has(.steps-grid),body>section:has(.pricing-grid){display:block!important;visibility:visible!important;opacity:1!important;padding:3rem 1.25rem!important;background:#0B0C0F!important}"
+"section:has(.roles-grid),[data-dg-hidden=roles-simplify]{display:none!important}"
+".trust-header,.roles-header{max-width:40rem;margin:0 auto 1.5rem!important;text-align:left!important}"
+".trust-header h2,.heading_tertiary,section h2{color:#F7F3EB!important;font-size:clamp(1.35rem,3vw,1.75rem)!important;font-weight:650!important;letter-spacing:-.02em!important;margin:.5rem 0 .4rem!important}"
+".trust-header p,.paragraph_large{color:#8E887F!important;font-size:1rem!important;max-width:36ch}"
+".steps-grid{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:1rem!important;max-width:52rem;margin:0 auto!important;visibility:visible!important;opacity:1!important}"
+".step-card{background:rgba(255,255,255,.03)!important;border:1px solid rgba(255,255,255,.08)!important;border-radius:16px!important;padding:1.15rem 1.1rem!important;box-shadow:none!important;transition:border-color .15s ease}"
+".step-card:hover,.step-card:focus-within{transform:none!important;border-color:rgba(232,217,168,.35)!important}"
+".step-num{color:#D4B56A!important;font-size:.75rem!important;font-weight:700!important;letter-spacing:.12em!important;margin-bottom:.45rem!important}"
+".step-title{color:#F7F3EB!important;font-size:1.05rem!important;font-weight:650!important;margin:0 0 .35rem!important}"
+".step-desc{color:#8E887F!important;font-size:.92rem!important;line-height:1.45!important;margin:0!important}"
+".pricing-grid,.pricing-grid.is-single{display:block!important;max-width:26rem;margin:0 auto!important;padding:0!important;visibility:visible!important}"
+".pricing-card{background:rgba(255,255,255,.03)!important;border:1px solid rgba(255,255,255,.1)!important;border-radius:18px!important;padding:1.6rem 1.4rem!important;box-shadow:none!important;text-align:left!important}"
+".pricing-card h3{color:#8E887F!important;letter-spacing:.1em!important;font-size:.72rem!important;text-transform:uppercase!important;margin:0 0 .35rem!important}"
+".pricing-card .pricing-amount,.pricing-card .price,.pricing-card > div > div{color:#F7F3EB!important}"
+".pricing-card a.premium-btn,.pricing-card a.button{margin-top:1.1rem!important;min-height:48px!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border-radius:999px!important;background:#E8D9A8!important;color:#12141A!important;border:none!important;padding:.7rem 1.2rem!important;font-weight:650!important}"
+".pricing-card-border-red,.pricing-card-divider{opacity:.35}"
+"#dg-legal-links{display:flex!important;flex-wrap:wrap!important;justify-content:center!important;gap:.15rem .9rem!important;margin:.75rem auto!important;max-width:28rem!important;padding:0 .75rem!important}"
+"#dg-legal-links a{color:#8E887F!important;text-decoration:none!important;margin-right:0!important;font-size:.85rem!important;min-height:44px!important;display:inline-flex!important;align-items:center!important}"
+"#dg-legal-links a:hover,#dg-legal-links a:focus-visible{color:#E8D9A8!important}"
+"#footer-email,footer a[href^=mailto]{color:#D4B56A!important;text-decoration:none!important;border-bottom:none;min-height:44px!important;display:inline-flex!important;align-items:center!important}"
+"#footer-email:hover,footer a[href^=mailto]:hover{color:#E8D9A8!important}"
...
+"footer,.footer{padding:2rem 1rem 2.5rem!important;text-align:center!important;border-top:1px solid rgba(255,255,255,.06)!important;max-width:32rem;margin-inline:auto;background:transparent!important}"
+"#demigod-footer-tag,footer .footer-tagline{font-size:.88rem!important;color:#6F6A63!important;max-width:26rem;margin:.35rem auto!important;line-height:1.45!important}"
+"#dg-copyright,footer [class*=copyright],footer .text-color_secondary{color:#5C5852!important;font-size:.8rem!important;margin:.5rem auto 0!important}"
+"#main,main,h1.hero-title,.hero-section h1,#dg-page{scroll-margin-top:4.5rem!important}"
+"a:focus-visible,button:focus-visible,.premium-btn:focus-visible,#dg-bar a:focus-visible{outline:2px solid #E8D9A8!important;outline-offset:3px!important}"
+"@media(max-width:767px){.hero-actions{display:none!important}#dg-bar{position:fixed!important;left:0;right:0;bottom:0;z-index:9998;display:grid!important;grid-template-columns:1fr 1fr;gap:8px;padding:10px 12px calc(10px + env(safe-area-inset-bottom,0px))!important;background:rgba(11,12,15,.97)!important;border-top:1px solid rgba(255,255,255,.08)!important}#dg-bar a{min-width:0!important;min-height:48px!important;border-radius:999px!important;font-size:.9rem!important;font-weight:650!important;padding:.65rem .5rem!important;display:flex!important;align-items:center!important;justify-content:center!important;text-decoration:none!important}body{padding-bottom:calc(72px + env(safe-area-inset-bottom,0px))!important}.steps-grid{grid-template-columns:1fr!important}.hero-section h1,.header h1{font-size:clamp(2rem,9vw,2.75rem)!important}#dg-path-pills{justify-content:flex-start}}"
+"@media(min-width:768px){#dg-bar{display:none!important}body{padding-bottom:0!important}}"
+"@media(prefers-reduced-motion:reduce){.hero-section h1,.hero-section p,.hero-actions,#dg-hero-chips,#dg-path-pills,.step-card{animation:none!important;transition:none!important}}"
...
window.__dgFootVer='564';console.log('Demigod v564');
