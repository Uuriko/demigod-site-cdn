...
/* 2026-08-14: home copy — SF Bay Area roles; Sign up to Demigod; process less-is-more. */
window.dgFootVersion = 'v1102'; console.log('[demigod] foot v1102-core loaded');
(function(){
...
function hero(){
  ...
  // Home process: a person picks · both say yes (do not restate software/compares/propose)
  qa('.step-card').forEach(function(card,i){
    ...
    var steps=[
      ['Send a brief','Role, must-haves, cash. Nobody\'s messaged yet.'],
      ['A person picks','They read the match and can say why.'],
      ['You both say yes','Intro only after both sides agree.']
    ];
  ...
  qa('.trust-header h2,.trust-section h2,section h2').forEach(function(h){
    ...
    /* also catch the retired software/compares/propose H2 so re-runs pick up the new voice */
    if(/HUMAN-MATCHED|TECH-MATCHED|TECH MATCHED|THE PROCESS|How it works|SF STARTUP TALENT|three clear steps|A match has three|Software compares/i.test(x) && h.closest('.trust-section,section:has(.steps-grid),section')) h.textContent='A person picks. You both say yes.';
  ...
  qa('.trust-header p,.trust-section .paragraph_large').forEach(function(p){
    ...
    if(/spam|profile|Bay Area|theater|steps|form|tech|human|Software compares|A person picks/i.test(p.textContent||'')){
      p.textContent='';
      try{p.style.setProperty('display','none','important');p.setAttribute('aria-hidden','true')}catch(e){}
    }
  });
  ...
  // one calm trust line (not chip salad; empty COPY.heroTrustLine = no second paragraph)
  ...
  if(chips&&!COPY.heroTrustLine){
    try{chips.remove()}catch(e){}
  }else if(chips&&COPY.heroTrustLine){
...
function injectNightHero(){
  ...
  try{killGoldTheater()}catch(e){}
}
/* Live canvas 2026-08-13: gold H1 / statue SVG / fake roles / contact-scrub still in HTML.
   Remove nodes so they cannot paint. Do not write the retired mailbox string into this file. */
function killGoldTheater(){
  ...
  qa('#dg-contact-strip,#insights-section,.insights-section,[class*="get-in-touch"],[class*="contact-us"]').forEach(function(el){
    try{el.remove()}catch(e){}
  });
  var scrub=q('#dg-contact-scrub');
  if(scrub) try{scrub.remove()}catch(e){}
}
...
function scrubStaticLabels(){
  ...
  qa('h2').forEach(function(h){
    ...
    if(/three clear steps|A match has three|THE PROCESS|^How it works$|Software compares\. A human proposes/i.test(tx))
      h.textContent='A person picks. You both say yes.';
    else if(/Examples of roles we can help with|^Example roles$/i.test(tx))
      h.textContent='Example roles — labeled samples';
    if(h.closest&&(h.closest('section.trust-section,section:has(.steps-grid),section:has(.pricing-grid),.trust-header,.pricing-section')||/Nothing until a hire|Software compares|A person picks|Example roles|Open roles/i.test(policyText(h.textContent)))){
      try{
...
window.__dgFootVer='1101';console.log('Demigod v1101');
