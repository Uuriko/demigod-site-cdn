 ...
 */
window.dgFootVersion='v613'; console.log('[demigod] foot v612-core loaded');
(function(){
...
var DG_PAGES = {
  ...
  mud: {
    ...
    desc: 'Social SF text multiplayer for site visitors — talk, walk SoMa, ask Codex the Immortal. Skippable tutorial.',
    html:
      '<p class="dg-p-lead">A <strong>full-page</strong> social text MUD set in <strong>San Francisco</strong>. Primary goal: meet other visitors. Lightweight exploration + <strong>Codex the Immortal</strong> (DM) if you get stuck. Short tutorial — type <code>skip</code> anytime.</p>' +
      '<ul class="dg-p-list">' +
      ...
      '<li><strong>Codex</strong> — immortal DM: <code>codex hint</code> · <code>codex quest</code></li>' +
      ...
      '<p class="dg-p-note">Solo works offline. Multiplayer + live Codex AI: <code>node demigod-mud-app.mjs</code> on :3461 (OPENAI_API_KEY optional). SF startup names = flavor only.</p>',
  },
 ...
 * Offline solo + NPCs + Codex immortal. Optional multi :3461.
 *
...
(function (root, factory) {
  ...
  var ROOMS = {
    ...
    roof: {
      name: 'SoMa Roof Deck',
      desc: 'Wind off the bay. Neon and cranes. Sutro Tower a faint trident. Immortal Codex is often "nearby" in spirit.',
      exits: { down: 'lobby' },
      items: ['lantern'],
    },
    mission: {
  ...
  var ITEMS = {
    leaflet: { name: 'a phosphor SF leaflet', look: 'create · walk · talk · codex if stuck · skip tutorial anytime. SF startups = flavor only.' },
    ...
    quest_note: { name: 'a quest slip from Codex', look: 'Immortal handwriting: optional, skippable, fun.' },
    coffee: { name: 'a warm coffee', look: 'Codex conjured. Flavor only.' },
  ...
  var NPCS = [
    {
      ...
      lines: [
        ...
        'Codex is the Immortal — ask them if stuck: codex hint',
      ],
    },
    {
    ...
    },
    {
      ...
      lines: [
        ...
        'Codex hangs around when the multiplayer relay is lonely.',
      ],
    },
    {
      id: 'codex', name: 'Codex', room: 'roof', immortal: true,
      look: 'An Immortal presence — dungeon master, not a mortal PC. Calm frege phosphor. Type: codex <message>',
      lines: [
        'I am Codex the Immortal. Ask for a quest, a hint, or company.',
  ...
  var TUTORIAL = [
    ...
    'Tutorial 4/4 — stuck? CODEX hint · or CODEX quest. Done — have fun.',
  ];
  ...
  Engine.prototype.boot = function () {
    ...
    this.print('Social text multiplayer for site visitors — walk, talk, meet. Lightweight game layer. HELP · SKIP tutorial · CODEX if stuck.', 'sys');
    if (ch) {
      this.char = ch;
  ...
  Engine.prototype.peopleHere = function () {
    ...
    // Codex always "reachable"
    if (list.indexOf('Codex (Immortal)') < 0 && list.indexOf('Codex') < 0) {
      list.push('Codex (Immortal · type codex …)');
    }
    return list;
  };
  ...
  Engine.prototype.lookRoom = function (verbose) {
    ...
    if (verbose) this.print('(HELP · MAP · SAY · CODEX · SKIP tutorial)', 'sys');
  };
  ...
  Engine.prototype.skipTutorial = function () {
    ...
    saveChar(this.char);
    this.print('Tutorial skipped. HELP · MAP · SAY · CODEX anytime.', 'sys');
  };
  ...
  Engine.prototype.applyCodexActions = function (actions) {
    ...
    (actions || []).forEach(function (a) {
      ...
      switch (a.type) {
        case 'say':
          self.print(a.text || 'Codex says something.', 'say');
          break;
        case 'emote':
          self.print('Codex ' + (a.text || 'gestures.'), 'say');
          break;
        case 'give': {
          var id = a.item;
          if (ITEMS[id] && self.char.inv.indexOf(id) < 0) {
            ...
            saveChar(self.char);
            self.print('Codex gives you ' + ITEMS[id].name + '.', 'sys');
          }
          break;
        }
        case 'quest':
        ...
        case 'move':
          if (a.room && ROOMS[a.room]) {
            ...
            saveChar(self.char);
            self.print('Codex relocates you…', 'sys');
            self.lookRoom(false);
            self.syncRoom();
          }
          break;
  ...
  Engine.prototype.askCodex = function (msg) {
    var self = this;
    if (!msg) {
      this.print('Usage: codex <message>  — or: ask codex <message>', 'err');
      return;
    }
    this.print('You address Codex, the Immortal…', 'sys');
    this.tutorialTick('codex');
    ...
    function offline() {
      ...
      if (/who are you|what are you|immortal/.test(m)) {
        reply = 'I am Codex — Immortal (wizard/DM) of this district. Not a mortal player. Ask for quests, hints, or company.';
        ...
        reply = 'Hello, ' + self.char.name + '. Talk with SAY / EMOTE / CHAT. I will help if you get stuck.';
      } else if (/coffee|ritual/.test(m)) {
        reply = 'Coffee, then.';
        actions = [{ type: 'give', item: 'coffee' }];
      } else {
        reply = 'I hear you. Try: quest · hint · who are you · or ask about a room. Multiplayer: SAY when the status says multi.';
      }
      self.print('Codex (Immortal): ' + reply, 'say');
      self.applyCodexActions(actions);
      self._codexHist.push({ role: 'user', content: msg });
      self._codexHist.push({ role: 'assistant', content: reply });
    }
    ...
    fetch(this.apiBase + '/codex', {
      ...
      body: JSON.stringify({
        ...
        message: msg,
        context: ctx,
        history: this._codexHist.slice(-8),
      }),
      signal: AbortSignal.timeout(45000),
    })
      .then(function (r) { return r.json(); })
      .then(function (j) {
        if (j && j.reply) {
          self.print('Codex (Immortal): ' + j.reply, 'say');
          self.applyCodexActions(j.actions || []);
          self._codexHist.push({ role: 'user', content: msg });
          self._codexHist.push({ role: 'assistant', content: j.reply });
          if (self._codexHist.length > 16) self._codexHist = self._codexHist.slice(-16);
        } else offline();
      })
  ...
  Engine.prototype.cmd = function (raw) {
    ...
    // codex / ask codex / immortal
    var codexM = line.match(/^(?:codex|immortal|ask\s+codex)\s+([\s\S]+)$/i);
    if (codexM) return this.askCodex(codexM[1]);
    if (/^(codex|immortal)$/i.test(line)) return this.askCodex('who are you');
    ...
    switch (verb) {
      ...
      case 'talk':
        ...
        if (/token for the bay/i.test(arg) && ch.room === 'dock' && ch.inv.indexOf('coin') >= 0) {
          ...
          this.print('The bay seems to approve. Codex whispers: quest complete — optional XP for talking to the city.', 'sys');
          ch.xp = (ch.xp || 0) + 5;
      ...
      case 'to': {
        ...
        var target = parts[1];
        var msg = parts[2];
        if (/^codex$/i.test(target)) return this.askCodex(msg);
      ...
      case 'players': {
        var lines = (this.remoteWho && this.remoteWho.length)
          ...
          : ['(solo — start multi with node demigod-mud-app.mjs, or just talk to NPCs/Codex)'];
      ...
      case 'journal':
        ...
        else this.print('No active quest. Ask: codex quest', 'meta');
        return;
      case 'score':
      case 'stats':
      case 'status':
      ...
      case 'tutorial':
        this.print(
          'Social: say · emote/me · wave · intro · chat · tell <name> · who\n' +
          'Move: look (l) · n/s/e/w/u/d · map\n' +
          'Stuff: inv · get · drop · examine (x) · score · quest\n' +
          'Immortal: codex <msg> · ask codex <msg>  (hints, quests, dynamic help)\n' +
          'Other: help · skip (tutorial) · quit · restart\n' +
          'Goal: meet people. Game bits are optional. Multi when status says multi.',
          'sys'
        );
        return;
      case 'quit':
      ...
      case 'hello':
        ...
        return;
      default:
        this.print("Unknown. Type HELP — or CODEX hint if stuck.", 'err');
    }
  };
  ...
  Engine.prototype.examine = function (arg) {
    ...
    if (/^codex$/.test(a)) {
      this.print(NPCS.find(function (n) { return n.id === 'codex'; }).look, 'out');
      return;
    }
    ...
    if (a === 'self' || a === 'me') return this.cmd('score');
  ...
  function mount(root, opts) {
    ...
    box.innerHTML =
      ...
      'placeholder="say hi · map · codex hint · help · skip" enterkeyhint="send" />' +
      '<button type="submit" class="dg-mud-send">Enter</button>' +
      '</form>' +
      '<p class="dg-mud-hint">SF social MUD · talk first · <code>codex</code> Immortal · <code>skip</code> tutorial · multi on :3461</p>' +
      '</div>';
  ...
  function selftest() {
    ...
    var ok =
      ...
      lines.some(function (l) { return /Immortal|Codex/i.test(l[1]); }) &&
      lines.some(function (l) { return /Social:|MAP/i.test(l[1]); });
    if (!ok) {
...
window.__dgFootVer='613';console.log('Demigod v612');
