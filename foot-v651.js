 ...
 */
window.dgFootVersion = 'v651'; console.log('[demigod] foot v647-core loaded');
(function(){
...
function openPage(id, push) {
  ...
  // Blog page: render cards from DG_BLOG_POSTS + deep-link Full note
  if (id === 'blog') {
    try {
      blogPageMount(root);
    } catch (eBlog) {}
...
function focusBlogNoteFromHash(root) {
  try {
    ...
    try {
      ...
      if (nt) {
        document.title = nt + ' · Blog · Demigod';
        var ogT=document.querySelector('meta[property="og:title"]'); if(ogT)ogT.setAttribute('content',nt+' · Demigod Blog');
        var twT=document.querySelector('meta[name="twitter:title"]'); if(twT)twT.setAttribute('content',nt+' · Demigod Blog');
      }
      if (nd) {
...
window.__dgFootVer='651';console.log('Demigod v647');
