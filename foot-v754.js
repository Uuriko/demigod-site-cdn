 ...
 */
window.dgFootVersion = 'v754'; console.log('[demigod] foot v754-core loaded');
(function(){
...
var WIZ_CFG={
  startup:{
    steps:[['welcome'],['company-name'],['company-stage'],['role-title'],['stack-needs'],['90day-outcome'],['salary-range'],['contact-email'],['__submit__'],['__thanks__']],
    ...
    thanks:STARTUP_OK,
    optional:['phone','why-this-role','role-jd','timeline','team-size']
  },
  engineer:{
    steps:[['welcome'],['full-name'],['seeker-email'],['linkedin-url'],['skills-stack'],['experience'],['links'],['sf-bay'],['availability'],['salary-expectation'],['resume'],['__submit__'],['__thanks__']],
    ...
    thanks:ENGINEER_OK,
    optional:['linkedin-url','phone','links','why-startups']
  }
};
...
var WIZ_Q={
  ...
  engineer:{
    ...
    'linkedin-url':{q:'LinkedIn URL? (optional)',h:'Helpful for context, but your work matters most. You can skip this.'},
    ...
    'salary-expectation':{q:'What cash compensation range are you targeting?',h:'A rough base range is enough; add equity context if useful.'},
    'why-startups':{q:'Why startups? (optional)',h:'One line if you want.'},
    'links':{q:'Work sample or profile? (optional)',h:'Portfolio, writing, campaign, GitHub, or any link that shows the work.'},
    'phone':{q:'Phone? (optional)',h:'SMS is pending. Email is the main channel.'},
    'resume':{q:'Resume?',h:'Upload a PDF/Word resume or paste a shareable resume link. Shared only after both sides approve.'},
...
function talentProofPrompt(skills, canUpload){
  return {q:'Resume?',h:(canUpload?'Upload a PDF/Word resume or paste':'Paste')+' a shareable resume link. Shared only after both sides approve.'};
}
...
function dgFormAnalytics(kind, key, event) {
  try {
    ...
    var stepMap = {
      ...
      'stack-needs':'skills', '90day-outcome':'outcome', 'salary-range':'constraints', 'contact-email':'contact',
...
if(nativeResume){
  ...
  if(!rw){
    ...
    rw.innerHTML='<label class="w-form-label" for="resume-url">Resume link *</label><input class="w-input" type="url" id="resume-url" name="resume-url" autocomplete="url" maxlength="2048" required placeholder="https://… shareable resume link"><p class="dg-resume-hint">Paste a shareable resume link.</p>';
...
var DG_PAGES = {
  ...
  faq: {
    ...
    html:
      ...
      '<details class="dg-p-det"><summary>What do I need as talent?</summary><p>Name, email, core strengths, work highlights, availability, cash compensation range, and a resume file or shareable resume link. LinkedIn and work samples are optional. Free forever — outreach only on real fits.</p></details>' +
  ...
  map: {
    title: 'SF startup map',
    doc: 'SF startup map · Demigod',
    desc: 'A privacy-safe, open-data neighborhood map of recent technology business activity in San Francisco.',
    html: '<div id="dg-startup-map" aria-live="polite"><p class="dg-p-lead">Loading the SF startup map…</p></div>',
  },
  method: {
    title: 'Method',
    doc: 'Method · Demigod',
...
'<p class="dg-p-note">LIVE = real visitors can hear you. Solo still works with NPCs. SF names are flavor only.</p>',
  ...
  events: {
    ...
    desc: 'Imagine a feasible San Francisco event and get a practical private draft plan in one click.',
    html:
      '<p class="dg-p-lead"><strong>One click, one event plan.</strong> Leave the direction blank and Events Bot will invent the room, audience, format, timing, and first operating draft.</p>' +
      '<p class="dg-ev-pill" role="note">San Francisco · in person · private draft</p>' +
      '<p class="dg-p-note dg-ev-trust">Nothing is sent, published, booked, or charged. Do not add private guest or contact information here.</p>' +
      '<div id="dg-events-chat" class="dg-events-chat" aria-label="Events Bot event planner">' +
      '<div class="dg-ec-head"><span class="dg-ec-title">Create an SF event</span><span class="dg-ec-status" id="dg-ec-status" aria-live="polite">Ready</span></div>' +
      '<form class="dg-ec-form" id="dg-ec-form">' +
      '<div class="dg-ev-prompt"><label class="dg-ev-lab" for="dg-ec-input">Optional direction</label>' +
      '<textarea id="dg-ec-input" class="dg-ec-input" rows="3" maxlength="2000" placeholder="Leave blank for a surprise, or add a vibe, audience, size, or constraint"></textarea></div>' +
      '<button type="submit" class="dg-ec-send" id="dg-ec-send">Imagine my event</button>' +
      '</form>' +
      '<div class="dg-ec-log" id="dg-ec-log" role="log" aria-live="polite" aria-label="Event plan"></div>' +
      '</div>' +
      '<details class="dg-p-det"><summary>What happens after the draft?</summary><p>Events Bot can turn the concept into a venue plan, agenda, guest framing, and ready-to-paste Demigod, Luma, or Partiful materials. External publishing and outreach stay separate and explicit.</p></details>' +
...
function pageCss() {
  ...
  s.textContent =
    ...
    '@media(max-width:520px){#dg-page .dg-ev-cal-row,#dg-page .dg-ec-form{grid-template-columns:1fr}#dg-page .dg-ec-send{width:100%}#dg-page .dg-ev-cal-cell{min-height:2.6rem;font-size:.75rem}}' +
    ...
    .then(function (j) {
      ...
      if (metaEl) {
        metaEl.textContent =
          ...
          (ev.rsvpYes != null ? ' · ' + ev.rsvpYes + ' yes' : '') +
          (ev.rsvpOpen ? ' · RSVP open' : ' · RSVP closed');
      }
      if (bodyEl) {
        bodyEl.textContent = '';
        [
      ...
      if (form && !ev.rsvpOpen) {
        ...
        setMsg('RSVPs are not open for this night yet.', 'err');
      }
    })
    .catch(function () {
...
function startupMapMount(root) {
  ...
  if(!dgStartupMapLoading){
    dgStartupMapLoading=new Promise(function(resolve,reject){
      ...
      script.src=DG_STARTUP_MAP_ASSET;
      script.async=true;
...
function openPage(id, push) {
  ...
  if (id === 'events') {
    try { eventsBotChatMount(root); } catch (e) {}
  }
  if (id === 'event') {
    try {
...
window.__dgFootVer='754';console.log('Demigod v754');
