 ...
 */
window.dgFootVersion = 'v681'; console.log('[demigod] foot v681-core loaded');
(function(){
...
/** Codex: Unbounded for brand wordmark (loads even if head paste lag). */
function ensureCyberFont() {
  if (q('#dg-cyber-font')) return;
  try {
    var link = document.createElement('link');
    link.id = 'dg-cyber-font';
    link.rel = 'stylesheet';
    link.href =
...
function pageCss() {
  ...
  if (existing && existing.getAttribute('data-v') === '681') return;
  if (existing) existing.remove();
  var s = document.createElement('style');
  s.id = 'dg-page-css';
  s.setAttribute('data-v', '681');
  s.textContent =
...
window.__dgFootVer='681';console.log('Demigod v681');
