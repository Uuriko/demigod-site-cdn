/*dg-foot-v1100-core*/
window.dgFootVersion = 'v1100'; console.log('[demigod] foot v1100-core loaded');
(function(){
...
function bountyRenderId(root) {
  ...
  var x = bountyXUser();
  if (gh) {
    host.innerHTML = '<img class="dg-bounty-av" src="' + esc(gh.avatarUrl || '') + '" alt="" width="28" height="28" />' +
      '<a class="dg-bounty-handle" href="' + esc(gh.htmlUrl || ('https://github.com/' + gh.login)) + '" rel="noopener" target="_blank">' + esc(gh.login) + '</a>' +
      (x ? ('<a class="dg-bounty-handle" href="' + esc(x.htmlUrl || ('https://x.com/' + x.handle)) + '" rel="noopener" target="_blank">@' + esc(x.handle) + '</a>') : '<button type="button" class="dg-bounty-x" id="dg-bounty-x" hidden>X</button>');
  } else {
    host.innerHTML = '<button type="button" class="dg-bounty-gh" id="dg-bounty-gh">GitHub</button>' +
      '<button type="button" class="dg-bounty-x" id="dg-bounty-x" hidden>X</button>';
  }
  var ghBtn = host.querySelector('#dg-bounty-gh');
  if (ghBtn) ghBtn.addEventListener('click', function () { bountyGhStart(); });
  ...
  bountyOauth('/oauth/github/status').then(function (r) { return r.json(); }).then(function (j) {
    if (j && j.x && !x && xBtn) xBtn.hidden = false;
  }).catch(function () {});
}
function bountyGhStart() {
...
function bountyRender(root, listings) {
  ...
  host.querySelectorAll('[data-dg-bounty-pay],[data-dg-bounty-copy]').forEach(function (el) {
    el.addEventListener('click', function (ev) {
      if (!bountyGhUser()) { ev.preventDefault(); bountyGhStart(); return; }
...
window.__dgFootVer='1100';console.log('Demigod v1100');
